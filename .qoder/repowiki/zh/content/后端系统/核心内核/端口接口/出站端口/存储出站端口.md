...
- LocalObjectStorageAdapter：基于本地文件系统的实现，提供 ensureBucket、upload/reliableUpload、openStream、deleteByUrl 的具体逻辑。
...
- 设计要点
  ...
  - upload/reliableUpload：上传接口；可靠上传在语义上与普通上传一致（当前实现未区分），便于未来扩展。
...
- 关键实现
  ...
  - upload/reliableUpload：生成唯一文件名（UUID 前缀 + 原始文件名），写入目标路径，返回 StoredObject。
...
- 关键实现
  ...
  - upload/reliableUpload：生成唯一键（UUID + 后缀），设置内容类型，调用 SDK 写入对象，返回 StoredObject。
