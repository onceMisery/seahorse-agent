# 调试技巧

<cite>
**本文引用的文件**
- [application.properties](file://seahorse-agent-bootstrap/src/main/resources/application.properties)
- [application.yml](file://seahorse-agent-mcp-server/src/main/resources/application.yml)
- [日志管理.md](file://docs/zh/content/监控运维/日志管理.md)
- [性能优化.md](file://docs/zh/content/数据库设计/性能优化.md)
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [空观测适配器](file://seahorse-agent-adapter-observation-noop/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/noop/NoopObservationAdapter.java)
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)
- [MCP HTTP 适配器属性](file://seahorse-agent-adapter-mcp-http/src/main/java/com/miracle/ai/seahorse/agent/adapters/mcp/http/McpHttpAdapterProperties.java)
- [JdbcRagTraceRepositoryAdapter.java](file://seahorse-agent-adapter-repository-jdbc/src/main/java/com/miracle/ai/seahorse/agent/adapters/repository/jdbc/JdbcRagTraceRepositoryAdapter.java)
- [seahorse_init.sql](file://resources/database/seahorse_init.sql)
- [DashboardPage.tsx](file://frontend/src/pages/admin/dashboard/DashboardPage.tsx)
- [docker-compose.yml](file://docker-compose.yml)
- [docker-compose.full.yml](file://docker-compose.full.yml)
- [milvus-stack-2.6.6.compose.yaml](file://resources/docker/milvus-stack-2.6.6.compose.yaml)
- [pulsar-stack-3.1.3.compose.yaml](file://resources/docker/pulsar-stack-3.1.3.compose.yaml)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本指南面向 Seahorse Agent 多模块 Spring Boot 工程，提供从日志分析、断点调试、远程调试、网络抓包、数据库查询调试、内存与性能分析到前端调试的全链路调试技巧。文档基于现有仓库中的配置与实现，结合最佳实践，帮助快速定位问题、优化性能并提升开发效率。

## 项目结构
项目采用 Maven 多模块结构，核心模块包括：
- bootstrap：应用启动与基础配置
- kernel：领域内核与业务能力
- adapter-*：适配器与外部集成（消息队列、存储、观测、MCP 等）
- starter：自动装配与可选组件打包
- frontend：前端工程（与后端日志管理相对独立）

```mermaid
graph TB
subgraph "后端服务"
BOOT["bootstrap<br/>应用启动"]
KERN["kernel<br/>内核与业务"]
AD_OBS_M["adapter-observation-micrometer<br/>观测适配器(Micrometer)"]
AD_OBS_N["adapter-observation-noop<br/>观测适配器(Noop)"]
AD_WEB["adapter-web<br/>Web 控制器/适配器"]
AD_MQ_P["adapter-mq-pulsar<br/>Pulsar 消息队列"]
AD_MCP_H["adapter-mcp-http<br/>MCP HTTP 适配器"]
end
subgraph "前端应用"
FE["frontend<br/>React/Vite 应用"]
end
BOOT --> KERN
KERN --> AD_OBS_M
KERN --> AD_OBS_N
KERN --> AD_WEB
KERN --> AD_MQ_P
KERN --> AD_MCP_H
FE -.-> BOOT
```

图表来源
- [pom.xml](file://pom.xml)
- [application.properties](file://seahorse-agent-bootstrap/src/main/resources/application.properties)
- [application.yml](file://seahorse-agent-mcp-server/src/main/resources/application.yml)

章节来源
- [pom.xml](file://pom.xml)
- [application.properties](file://seahorse-agent-bootstrap/src/main/resources/application.properties)
- [application.yml](file://seahorse-agent-mcp-server/src/main/resources/application.yml)

## 核心组件
- 观测与指标：通过 Micrometer 观测适配器输出计数器、定时器等指标，便于统一采集与告警。
- 分布式追踪：内核提供 RAG 追踪记录器与追踪模型，支持时长、错误清洗、追踪 ID 生成等。
- 入湖节点日志：内核定义入湖节点日志端口，适配器实现本地记录，便于审计与问题定位。
- 消息队列与超时：Pulsar 属性包含压缩类型、发送超时等参数，影响日志传输可靠性与性能。
- MCP 适配器：MCP HTTP 适配器提供调用超时与服务器列表配置，便于外部工具链日志对齐。

章节来源
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)
- [MCP HTTP 适配器属性](file://seahorse-agent-adapter-mcp-http/src/main/java/com/miracle/ai/seahorse/agent/adapters/mcp/http/McpHttpAdapterProperties.java)

## 架构总览
下图展示日志与观测在系统中的位置与交互：

```mermaid
graph TB
subgraph "应用层"
CTRL["Web 控制器/服务"]
PIPE["内核流水线/引擎"]
end
subgraph "观测与追踪"
OBS["Micrometer 观测适配器"]
TRACE["RAG 追踪记录器"]
PORT_LOG["入湖节点日志端口"]
ADAPTER_LOG["本地入湖节点日志适配器"]
end
subgraph "外部系统"
MQ["Pulsar 消息队列"]
METRICS["指标/告警系统"]
LOG_SYS["日志聚合系统(EFK/ELK)"]
end
CTRL --> PIPE
PIPE --> OBS
PIPE --> TRACE
PIPE --> PORT_LOG
PORT_LOG --> ADAPTER_LOG
PIPE --> MQ
OBS --> METRICS
TRACE --> LOG_SYS
ADAPTER_LOG --> LOG_SYS
MQ --> LOG_SYS
```

图表来源
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)

## 详细组件分析

### 观测与指标（Micrometer）
- 功能要点
  - 将命令与事件转化为指标标签，支持按租户、属性等维度打点。
  - 使用计数器记录事件，使用定时器采样耗时，便于统一采集与告警。
- 配置与扩展
  - 可通过引入 Micrometer 监控后端（如 Prometheus、InfluxDB）进行集中采集。
  - 适配器以 SPI 方式注册，可通过启用/禁用切换实现无侵入或降级。

```mermaid
classDiagram
class MicrometerObservationAdapter {
+start(command) ObservationScope
+recordEvent(event) void
}
class NoopObservationAdapter {
+start(command) ObservationScope
+recordEvent(event) void
}
class ObservationCommand {
+name() String
+tenantId() String
+attributes() Map
}
class ObservationEvent {
+name() String
+attributes() Map
}
class ObservationScope {
+recordEvent(event) void
+close() void
}
MicrometerObservationAdapter ..|> ObservationScope : "返回作用域"
NoopObservationAdapter ..|> ObservationScope : "返回空作用域"
MicrometerObservationAdapter --> ObservationCommand : "读取标签"
MicrometerObservationAdapter --> ObservationEvent : "计数事件"
```

图表来源
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [空观测适配器](file://seahorse-agent-adapter-observation-noop/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/noop/NoopObservationAdapter.java)

章节来源
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [空观测适配器](file://seahorse-agent-adapter-observation-noop/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/noop/NoopObservationAdapter.java)

### 分布式日志追踪（RAG 追踪）
- 功能要点
  - 记录开始/结束时间、耗时、错误信息（含清洗）、追踪 ID 等。
  - 支持对话 ID、任务 ID、用户 ID 等上下文字段，便于跨模块关联。
- 实现路径
  - 内核提供记录器与模型类，控制器或服务在关键路径调用。

```mermaid
sequenceDiagram
participant C as "控制器/服务"
participant R as "RAG 追踪记录器"
participant M as "追踪模型(RagTraceRun)"
C->>R : "开始追踪(名称, 方法, 上下文)"
R->>R : "记录开始时间/生成追踪ID"
C->>C : "执行业务逻辑"
C->>R : "结束追踪(结果/异常, 耗时)"
R->>R : "清洗错误信息/计算耗时"
R->>M : "构建追踪记录"
R-->>C : "返回追踪ID/记录"
```

图表来源
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核 RAG 追踪模型](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/trace/RagTraceRun.java)

章节来源
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核 RAG 追踪模型](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/trace/RagTraceRun.java)

### 入湖节点日志（审计与定位）
- 功能要点
  - 定义入湖节点日志端口，记录上下文、配置、结果与耗时。
  - 提供空实现与本地实现，便于在不同环境选择。
- 实施建议
  - 在 Web 适配器中接入本地实现，确保关键节点可审计。

```mermaid
flowchart TD
A["开始节点执行"] --> B["记录上下文/配置/开始时间"]
B --> C["执行节点逻辑"]
C --> D{"执行成功?"}
D -- 否 --> E["记录失败结果/异常信息"]
D -- 是 --> F["记录成功结果/耗时(ms)"]
E --> G["写入本地日志(适配器)"]
F --> G
G --> H["结束"]
```

图表来源
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)

章节来源
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)

### 消息队列与日志传输（Pulsar）
- 关键参数
  - 发送超时、阻塞策略、批处理、批延迟、压缩类型等。
- 对日志的影响
  - 影响日志投递的可靠性与时延，需结合观测指标与告警策略进行调优。

章节来源
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)

### 外部工具链日志（MCP HTTP）
- 关键参数
  - 启用开关、调用超时、服务器列表。
- 对日志的影响
  - 影响外部工具链的可用性与响应时间，应纳入观测与告警范围。

章节来源
- [MCP HTTP 适配器属性](file://seahorse-agent-adapter-mcp-http/src/main/java/com/miracle/ai/seahorse/agent/adapters/mcp/http/McpHttpAdapterProperties.java)

## 依赖关系分析
- 模块耦合
  - kernel 依赖多个 adapter，形成清晰的端口-适配器模式。
  - 观测适配器通过 SPI 注册，支持启用/禁用与替换。
- 外部依赖
  - Micrometer 为核心观测框架，建议配合指标系统统一采集。
  - POM 中声明了 Spring Boot 与各生态组件版本，保证兼容性。

```mermaid
graph LR
K["kernel"] --> O1["adapter-observation-micrometer"]
K --> O2["adapter-observation-noop"]
K --> W["adapter-web"]
K --> Q["adapter-mq-pulsar"]
K --> M["adapter-mcp-http"]
O1 --> MIC["Micrometer 核心"]
```

图表来源
- [pom.xml](file://pom.xml)
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)

章节来源
- [pom.xml](file://pom.xml)

## 性能考量
- 指标采样
  - 使用定时器采样关键路径耗时，避免高频日志带来的性能开销。
- 批处理与压缩
  - Pulsar 的批处理与压缩类型可降低网络与存储压力，但需权衡延迟。
- 超时与重试
  - MCP 与消息队列的超时配置直接影响吞吐与稳定性，应结合观测指标动态调整。

## 故障排查指南
- 观测指标缺失
  - 检查 Micrometer 适配器是否启用、SPI 是否正确注册、指标系统是否连通。
- 追踪信息不完整
  - 确认追踪记录器是否在关键路径被调用，上下文字段是否正确传入。
- 入湖节点日志未落盘
  - 检查日志端口实现是否为本地适配器，文件权限与磁盘空间是否充足。
- 消息队列积压
  - 结合发送超时、批处理参数与消费者速率，查看观测指标与告警。

章节来源
- [Micrometer 观测适配器](file://seahorse-agent-adapter-observation-micrometer/src/main/java/com/miracle/ai/seahorse/agent/adapters/observation/micrometer/MicrometerObservationAdapter.java)
- [内核 RAG 追踪记录器](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/kernel/application/trace/KernelRagTraceRecorder.java)
- [内核入湖节点日志端口](file://seahorse-agent-kernel/src/main/java/com/miracle/ai/seahorse/agent/ports/outbound/ingestion/IngestionNodeLogPort.java)
- [本地入湖节点日志适配器](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/local/LocalIngestionNodeLogAdapter.java)
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)

## 结论
本项目以 Micrometer 与内核追踪为核心，提供了可观测、可审计、可扩展的日志与追踪能力。对于日志配置、聚合与轮转归档，建议结合实际部署环境补充 logback/log4j2 配置与 ELK/EFK/Fluentd/Filebeat 集成；同时将观测指标与告警策略落地，实现异常检测、性能分析与关键业务告警。

## 附录

### 日志分析方法
- 日志级别设置
  - 生产环境建议将业务包设为 INFO，调试阶段临时提升至 DEBUG。
  - 对第三方依赖包单独分级，避免噪声干扰。
- 关键日志追踪
  - 在关键业务路径记录结构化日志，包含时间戳、级别、应用名、模块、线程、请求追踪 ID、消息体等字段。
  - 为追踪 ID 与会话 ID 提供上下文字段，便于跨服务串联。
- 异常堆栈分析
  - 基于日志关键词与正则表达式识别异常，结合阈值触发告警。
  - 对异常堆栈进行分类统计，定位高频异常与热点路径。
- 性能日志解读
  - 结合 Micrometer 指标与日志中的耗时信息，定位慢调用与热点路径。
  - 关注 P95/P99 延迟、错误率等关键指标，及时发现性能退化。

章节来源
- [日志管理.md](file://docs/zh/content/监控运维/日志管理.md)

### 断点调试与远程调试
- IDE 调试设置
  - 在 bootstrap 模块中设置断点，逐步执行关键路径。
  - 利用条件断点与日志断点，缩小问题范围。
- 远程调试参数
  - 通过 JVM 参数启用远程调试，连接到目标实例进行调试。
  - 在 docker-compose 中配置调试端口映射，便于容器内调试。
- 热部署配置
  - 使用 Devtools 或 JRebel 实现热部署，提高开发效率。
  - 在前端工程中启用 Vite 的热更新功能，实时预览更改。

章节来源
- [application.properties](file://seahorse-agent-bootstrap/src/main/resources/application.properties)
- [docker-compose.yml](file://docker-compose.yml)
- [docker-compose.full.yml](file://docker-compose.full.yml)

### 网络抓包与协议分析
- HTTP 请求分析
  - 使用浏览器开发者工具的 Network 面板，查看请求与响应详情。
  - 关注状态码、响应时间、请求头与响应头，定位接口问题。
- WebSocket 消息跟踪
  - 在 Network 面板中查找 WebSocket 连接，查看消息收发。
  - 记录消息内容与时间戳，分析消息顺序与完整性。
- 消息队列通信监控
  - 结合 Pulsar 的管理界面与日志，监控消息发送与接收情况。
  - 关注发送超时、批处理参数与消费者速率，排查积压问题。

章节来源
- [Pulsar 消息队列属性](file://seahorse-agent-adapter-mq-pulsar/src/main/java/com/miracle/ai/seahorse/agent/adapters/mq/pulsar/PulsarMessageQueueProperties.java)

### 数据库查询调试
- SQL 执行计划分析
  - 使用 EXPLAIN/EXPLAIN ANALYZE 查看执行计划，识别全表扫描与索引失效。
  - 结合数据库内置的执行计划缓存，对比不同查询的性能差异。
- 慢查询识别
  - 基于慢查询日志与性能指标，识别执行时间超过阈值的 SQL。
  - 分析慢查询的执行路径，优化 WHERE 条件与 JOIN 顺序。
- 事务冲突排查
  - 检查锁等待与死锁日志，定位事务冲突点。
  - 通过隔离级别与锁粒度调整，减少冲突发生。

章节来源
- [JdbcRagTraceRepositoryAdapter.java](file://seahorse-agent-adapter-repository-jdbc/src/main/java/com/miracle/ai/seahorse/agent/adapters/repository/jdbc/JdbcRagTraceRepositoryAdapter.java)
- [性能优化.md](file://docs/zh/content/数据库设计/性能优化.md)
- [seahorse_init.sql](file://resources/database/seahorse_init.sql)

### 内存分析与性能分析工具
- JProfiler/VisualVM 使用
  - 使用 JProfiler 进行 CPU 与内存分析，识别热点方法与内存泄漏。
  - 使用 VisualVM 监控堆内存、GC 活动与线程状态，定位性能瓶颈。
- Heap Dump 分析
  - 生成 Heap Dump 文件，使用 Eclipse MAT 或 VisualVM 分析对象存活与引用关系。
  - 关注大对象与长生命周期对象，评估内存回收效果。

章节来源
- [性能优化.md](file://docs/zh/content/数据库设计/性能优化.md)

### 前端调试技巧
- 浏览器开发者工具使用
  - 使用 Elements 面板检查 DOM 结构与样式，定位布局问题。
  - 使用 Console 面板查看错误与警告，分析脚本执行异常。
- 网络面板分析
  - 在 Network 面板中查看资源加载情况，分析加载缓慢的原因。
  - 关注请求耗时与资源大小，优化静态资源与接口响应。
- 性能面板监控
  - 使用 Performance 面板记录页面渲染与脚本执行过程，识别性能瓶颈。
  - 结合 Lighthouse 进行性能评分与改进建议。

章节来源
- [DashboardPage.tsx](file://frontend/src/pages/admin/dashboard/DashboardPage.tsx)

### 调试环境搭建与工具配置最佳实践
- 容器化调试
  - 使用 docker-compose 启动后端服务与依赖组件，便于复现问题。
  - 在 Milvus 与 Pulsar 的 compose 文件中配置日志与监控，便于问题定位。
- 日志聚合与轮转
  - 在后端应用中输出 JSON 日志，由 Filebeat/Fluentd 收集并转发至 Logstash/Fluentd，再写入 Elasticsearch。
  - Kibana 用于可视化与查询，结合观测指标建立仪表板。
- 安全与合规
  - 对敏感字段进行脱敏处理，避免泄露。
  - 限制日志文件与日志系统的访问权限，仅授权人员可查看。

章节来源
- [milvus-stack-2.6.6.compose.yaml](file://resources/docker/milvus-stack-2.6.6.compose.yaml)
- [pulsar-stack-3.1.3.compose.yaml](file://resources/docker/pulsar-stack-3.1.3.compose.yaml)
- [日志管理.md](file://docs/zh/content/监控运维/日志管理.md)