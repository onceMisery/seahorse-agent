...
- PostgreSQL 架构脚本：schema_pg.sql
- 初始数据脚本：init_data_pg.sql
...
```mermaid
graph TB
A["初始化脚本目录<br/>resources/database"] --> B["架构脚本<br/>schema_pg.sql"]
A --> C["初始数据脚本<br/>init_data_pg.sql"]
A --> D["升级脚本 v1.0->v1.1<br/>upgrade_v1.0_to_v1.1.sql"]
A --> E["升级脚本 v1.1->v1.2<br/>upgrade_v1.1_to_v1.2.sql"]
A --> F["升级脚本 v1.2->v1.3<br/>upgrade_v1.2_to_v1.3.sql"]
...
```mermaid
graph TB
S["架构脚本 schema_pg.sql"] --> P["PostgreSQL + pgvector"]
S --> T["业务表与索引"]
U["升级脚本"] --> T
A["应用配置"] --> K["内核迁移模式"]
V["PgVectorAdapter"] --> T
