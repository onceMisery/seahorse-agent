...
subgraph "数据与存储"
DBSCHEMA["数据库模式<br/>schema_pg.sql"]
INITDATA["初始化数据<br/>init_data_pg.sql"]
...
graph TB
subgraph "备份基线"
BASE_DB["数据库基线<br/>schema_pg.sql/init_data_pg.sql"]
...
subgraph "恢复执行"
RESTORE_DB["数据库恢复<br/>schema_pg.sql/init_data_pg.sql"]
...
### 数据库备份策略
- 全量备份：以 schema_pg.sql 为模式基线，init_data_pg.sql 为初始数据基线，作为每日全量备份的模板。
...
```mermaid
flowchart TD
START(["开始"]) --> FULL["生成全量备份<br/>schema_pg.sql + init_data_pg.sql"]
