...
`2026-06-08-implementation-details-review.md` is technically useful, but it is a point-in-time review of an older version of this companion document, not the current implementation contract. A legacy console or default PowerShell read may render valid UTF-8 Chinese text as mojibake. If display output looks corrupted, verify with a UTF-8-aware editor, byte/code-point scan, or `rg` before copying any text into fixtures.

Recheck result on 2026-06-09: the review's original P0/P1 findings were reasonable for the older document, but several proposed fixes are now superseded by current code. Do not execute the review literally. Treat it as review evidence to classify against the current branch:
...
- Remaining completion gate: current-branch code, focused tests, the 2026-06-09 real backend E2E run, Admin Inspector browser proof, Chat control browser proof, and historical Chat Workbench hydration now cover the backend tool/skill/artifact/SSE/replay path plus user-facing Tool Calls, Skills, Artifacts, and Cost recovery for persisted Chat messages. Keep the final goal gate explicit until the post-commit audit verifies the whole plan requirement by requirement.
...
- Accepted with current compatibility detail: Java keeps `StreamEventEnvelope.eventSeq` as `long`, while the Spring Jackson configuration serializes `Long` values as JSON strings for large-number safety. `frontend/src/types/index.ts` still models in-memory `StreamEventEnvelope.eventSeq` as `number`, so replay/admin entry points that consume backend JSON must normalize safe decimal strings at the boundary before sorting or deduping. Do not remove the string fixtures in `AgentInspectorPage.test.tsx` until the shared frontend wire type is updated to `number | string` and all call sites are audited.
...
- [ ] Final current-worktree E2E gate: backend and frontend both started from this branch; Chat UI selects `github-visual-project-intro`; the user selects at least one skill such as `image-generation`; at least one tool call occurs; at least one generated artifact is persisted; live SSE includes the generated artifact as `stream_event` and named `agent.artifact` when the stream remains connected; Workbench Tool Calls, Skills, Artifacts, and Cost/Quota render for the Chat UI-started assistant message during live or persisted recovery; `sa_agent_run_event_buffer` has rows for the fresh run; `/api/agent-runs/{runId}/events?afterSeq=0` returns ordered/dedupable events; SSE resume or admin replay proves event ordering and dedupe. Historical runs or Docker services built from older commits do not satisfy the live-generation parts of this gate.
  ...
  - Current-branch backend ran from the branch jar in Docker as `seahorse-backend-current-branch` on `http://127.0.0.1:19094`, because local Windows DNS resolved `apihub.agnes-ai.com` to loopback while Docker compose already pins the provider host through `extra_hosts`.
  ...
  This proves the real backend tool/skill/artifact/SSE/replay path, admin replay, Chat control entry point, and persisted Chat Workbench recovery for the current branch. The remaining completion work is a full post-commit audit of the complete plan and any additional fresh live UI flow needed by that audit, not a known missing Workbench tab implementation.
