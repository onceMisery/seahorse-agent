...
- `frontend/src/hooks/useStreamResponse.ts` currently contains mojibake in stream watchdog comments and at least one user-facing timeout error string. Task 2 must include this hook because it sits on the SSE sender/retry path.
...
**Why:** A 2026-06-08 review found the named chat/workbench files currently clean for the known mojibake code points, but `frontend/src/hooks/useStreamResponse.ts` contains mojibake in comments and a timeout error message on the SSE retry path. Keep this as a P0 guard because the runtime work will touch Chinese labels and prior project work has had encoding regressions.
...
- Minimal change: first reuse the existing `/rag/v3/chat?resumeRunId=...&lastEventSeq=...` stream path and `stream_event` envelopes; add a separate event-list endpoint only if tests prove the existing resume path cannot support admin replay/backfill.
...
- [ ] Reuse the existing resume stream path for chat backfill; implement or extend backend event list/snapshot only if fields are missing for admin replay.
...
Risk: implementing a new event-list endpoint before using the existing `resumeRunId`/`lastEventSeq` stream path can split recovery behavior across two owners.

Mitigation: Task 11 must prove whether the existing resume stream supports chat backfill first; add a separate endpoint only for admin replay gaps that the stream path cannot cover.
