...
12. ~~真实 BM25/vector/graph 后端质量评估：建立 golden cases，衡量 channel contribution、recall、precision、MRR。仍需扩展 `MemoryRetrievalPipelinePort` 暴露 per-channel attribution（当前 `MemoryContext` 已 fuse 后无法分辨来源），属于较大 API 改造，建议作为独立切片推进。~~ 已完成 channel attribution 暴露（`f320af2a`）：`MemoryContextAttribution` + `loadWithAttribution` default 方法 + `MemoryRecallEvaluationResult/Report` 新增 `channelHitCounts`，覆盖测试已通过；剩余的真实后端 golden harness 仍可作为独立切片推进。
