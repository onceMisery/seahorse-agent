...
12. ~~真实 BM25/vector/graph 后端质量评估：建立 golden cases，衡量 channel contribution、recall、precision、MRR。仍需扩展 `MemoryRetrievalPipelinePort` 暴露 per-channel attribution（当前 `MemoryContext` 已 fuse 后无法分辨来源），属于较大 API 改造，建议作为独立切片推进。~~ 已完成 channel attribution 暴露（`f320af2a`）+ 命名 profile 驱动的 golden harness（`2dd25977`）：`MemoryRecallGoldenHarnessInboundPort` + `MemoryRecallGoldenCaseRepositoryPort` + 默认 classpath 加载器 + `SeahorseMemoryRecallGoldenHarnessController`，CI 可直接 `POST /memories/recall-quality/golden/profiles/{name}/run`；剩余的真实后端 long-running fixture 仅是数据投入，不再有架构缺口。
13. ~~Compaction 深化：等 `MemoryCompactionServiceTests` 的并行改动归属明确后再接。~~ 已通过新增 `MemoryCompactionServiceObservationTests` + `MemoryCompactionService` 增加 6-arg 兼容构造函数完成（`fb26a6e3`）：`memory-compaction-group`（success/skipped/error + strategy）+ `memory-compaction-run`（success/empty/error）两个 ObservationEvent；现有 `MemoryCompactionServiceTests`（3 个）保持不变。
...
15. ~~分布式 idle-flush scheduler：在 cache-redis 模块补 `MemoryAggregationSchedulerPort` 的 Redisson 实现，`RScoredSortedSet` 存 `tenantId::sessionId`、score=epochMillis；`pollDueEntries(now, limit)` 原子 drain，避免双重 flush。~~ 已完成（`c0a26ccf`）。`RedisMemoryAggregationSchedulerPort` + starter 装配 + 8 个 Mockito 单测。
