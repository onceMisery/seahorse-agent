...
| Hardening batch: error traceId + honest classification | `ErrorResponseContractTests` (3/3); `SeahorseE2E*` probe `-Dtest=SeahorseE2E*` with group exclusion runs 0 tests; spotless on web/tests | PASS; `traceId` populated from MDC when tracing enabled, omitted otherwise; E2E excluded by `@Tag("e2e")` group in root pom; CI name-based exclusion removed |
| Hardening batch: security automation | `.github/dependabot.yml` (maven/npm/actions weekly) and `.github/workflows/security.yml` (gitleaks push/PR/daily) inspection | PASS; no continue-on-error; tracked Playwright console logs removed from the index |
| Hardening batch: UNKNOWN reconciliation | `ToolInvocationReconciliationServiceTests` (5/5: sibling propagation, no-sibling FAILED, no-key FAILED, grace cutoff, lock-held skip); `JdbcToolInvocationAuditRepositoryAdapterTests` 6/6 including the two new H2 cases; spotless kernel/jdbc/autoconfigure | PASS; UNKNOWN audit records now converge to the sibling terminal outcome or FAILED after the 30-minute grace under a distributed lock; job wired behind `seahorse-agent.kernel.tool-reconciliation-enabled` |
| Hardening batch: memory engine Builder collapse | focused memory suites (DefaultMemoryEnginePortTests 67, lifecycle/routing/review/observability/conflict = 106 total) and full kernel reactor `mvn -pl seahorse-agent-kernel -am test` | PASS; 926 kernel tests, 0 failures; 28 test call sites migrated to the Builder chain; `DefaultMemoryEnginePort` 1050 -> 599 lines; large classes >800 fell 16 -> 15 and the complexity baseline was ratcheted |
| Hardening batch: quarantine interceptor | full web module `mvn -pl seahorse-agent-adapter-web -am test` | PASS; 305 tests including 4 new `FeatureQuarantineInterceptorTests` cases (stable 403 when disabled, pass-through when enabled, non-quarantined path, segment-safe prefix matching) and gate default assertions for the six new features |
| Hardening batch: frontend lazy loading + error mapper | `npx tsc --noEmit` PASS; `npm run lint` PASS; `npm test` 208/208; `npm run build` PASS | PASS; main bundle 3,725 kB -> 2,712 kB (-27%, gzip 819 kB) with Ingestion/Sandbox/AgentInspector pages as separate chunks; two route-registration text tests updated to the lazy idiom with CRLF-normalized reads |
| Frontend chunk-warning elimination | root cause: stale vite.config.js shadowed vite.config.ts (vite resolves .js first); removed; functional manualChunks by library family + React.lazy for MermaidDiagram/CodeEditor | PASS; tsc clean, 217/217, build clean; index 78 kB (gzip 20.7 kB), no >500kB warning; mermaid 2.5MB is an audited lazy chunk (per-diagram chunks are internal to mermaid v11) |
| Second wave: billing aggregates | `BillLineItemRepositoryPort`->`BillRepositoryPort`, `PaymentCallbackLogRepositoryPort`->`PaymentOrderRepositoryPort`; kernel/JDBC/autoconfigure reactors | PASS; merged ports 6 ops each, single consuming services; ports 358 -> 356 (93/262/1); PortArchitectureTest green after ratchet |
...
| Core-capability hardening batch (P0-1/P0-2/P1-3 remainder of docs/优化实施计划.md) | `CachedRetrievalEngine` gains the maxEntries FIFO cap (tenant-scoped key was already in place); `DefaultContextWeaver` 3-arg weave merges ContextPack + memory sections into one shared budget instead of dropping memory when a pack is present; `DefaultMemoryRouter` routes PROFILE on every turn (keyword gate removed); new `CachedRetrievalEngineTests` (7), `DefaultMemoryRouterTests` (5), rewritten `DefaultContextWeaverContextPackTests` merge cases, updated `MemoryWorkflowRoutingTests`; full kernel reactor + tests-module memory suites; spotless on kernel/tests | PASS; kernel 940/940, MemoryWorkflowRoutingTests 11/11, HybridMemoryRecallPipelineTests 11/11, DefaultMemoryEnginePortTests 67/67, MemoryRetrievalPipelineTests 4/4; P1-1/P1-2/P1-5/P2-1/P2-2 were verified already implemented (P1-1 failure flag + concurrent supplyAsync; P1-2 embed throws on missing data; P1-5 rewriteQuery try/catch fallback; P2-1 TENANT_PLACEHOLDER; P2-2 rerank warn logs) |
| Agent team orchestration P1 (Supervisor/Team, Multi-Agent A2A design §6) | `AgentTeamInboundPort`+`AgentTeamRepositoryPort` (351 -> 353 feature-driven, §6.3 justification in port-inventory), `KernelAgentTeamService` (supervisor plan/dispatch/synthesize via handoff; DAG topological execution, fail-fast), `KernelAgentHandoffService.completeOnTerminal` (child-run completion writeback), JDBC adapter + `sa_agent_team`/`sa_agent_team_run` tables (V61), `SeahorseAgentTeamController` (5 endpoints, AGENT_HANDOFF gate); tests `KernelAgentTeamServiceTests` (7), `JdbcAgentTeamRepositoryAdapterTests` (2, H2), registry wiring tests (+2 incl. negative condition) | PASS; kernel 947/947, autoconfigure 141/141, PortArchitectureTest green after baseline ratchet (353 = 93/259/1, port files 786), spotless clean; synchronous member execution rides the chat engine (`ChatInboundPort.streamChat` is synchronous, same seam as `TaskOrchestrationService`), so no worker-claim integration is required for P1 |
