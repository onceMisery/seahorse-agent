...
### 2026-07-04 AgentScope Core Physical Split M4 Evidence Update

The first M4 physical split slice now separates AgentScope core execution into `seahorse-agent-adapter-agent-agentscope-core`. The new module owns the core executor, model bridge, tool bridge/factory compatibility type, prompt provider interface, core properties, run metadata contributor, observation support, and core/compat auto-configuration. The existing `seahorse-agent-adapter-agent-agentscope` module remains the compatibility aggregate for A2A, Nacos, config center, and Studio integrations, and now depends on the core module.

`AgentScopeReActAutoConfiguration` remains a compatibility entry point: in core-only classpaths it imports only core/observation configuration, while in the aggregate classpath it conditionally imports optional A2A/Nacos/config-center/Studio auto-configurations by class name. This preserves existing aggregate tests while allowing the core artifact to build without optional AgentScope extension dependencies.

Fresh evidence: the initial Maven RED failed because `seahorse-agent-adapter-agent-agentscope-core` was absent from the reactor; after the split, `.\mvnw.cmd -pl seahorse-agent-adapter-agent-agentscope-core -am "-Dtest=AgentScopeCoreModuleBoundaryTests" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed 1/1 and verified optional integration auto-configurations are not present in the core-only module classpath. Focused aggregate regressions passed 33/33 across core auto-configuration, A2A runner, tool bridge, model bridge, and properties. The full `.\scripts\agentscope-release-gate.ps1` passed with AgentScope adapter tests 111/111, `KernelChatAgentRunStoreTests` 19/19, application smoke packaging success, and final `AGENTSCOPE_RELEASE_GATE=PASS`.

Follow-up P1 gate hardening: the release gate now includes `AgentScopeCoreModuleBoundaryTests` in its explicit unit-test allowlist, so the core-only optional-integration boundary is proven by `.\scripts\agentscope-release-gate.ps1` instead of only by a separate focused command. `Dockerfile.backend` also copies `seahorse-agent-adapter-agent-agentscope-core/pom.xml` before the backend image `dependency:go-offline` layer, and `scripts/verify-smoke-contracts.ps1` plus `DeploymentEvidenceGateScriptContractTests` guard that backend Docker cache path. Fresh local evidence: `AgentScopeReleaseGateScriptContractTests` passed 1/1, `DeploymentEvidenceGateScriptContractTests` passed 2/2, `scripts/verify-smoke-contracts.ps1` passed, and `.\scripts\agentscope-release-gate.ps1` passed with `AgentScopeCoreModuleBoundaryTests` 1/1, AgentScope adapter tests 111/111, kernel run contracts 19/19, application smoke packaging success, and final `AGENTSCOPE_RELEASE_GATE=PASS`. Fresh full-Docker evidence: after confirming the Docker engine was available and direct `docker pull eclipse-temurin:21-jre` worked through `HTTP_PROXY=http://192.168.1.9:7890`, `docker compose -f docker-compose.full.yml build --build-arg HTTP_PROXY=http://192.168.1.9:7890 --build-arg HTTPS_PROXY=http://192.168.1.9:7890 backend` rebuilt `seahorse-agent-backend` with an in-image Maven reactor `BUILD SUCCESS` across 28/28 modules, including `seahorse-agent-adapter-agent-agentscope-core`. `docker compose -f docker-compose.full.yml up -d --no-deps --force-recreate backend` recreated `seahorse-backend`, health reached `healthy`, `.\scripts\e2e-backend-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -RuntimeProfile full-compose` passed 20/20, and `.\scripts\e2e-agentscope-smoke.ps1 -BaseUrl http://127.0.0.1:9090` passed 11/11 with AgentScope run `run_333047469058535424`, kernel run `run_333047488822095872`, and matching AgentScope/kernel SSE and snapshot contracts.

### 2026-07-05 Tool Gateway Rollout Audit Attribution P1 Fix

Tool Gateway requested-audit records now carry `rolloutId` end to end. The fix adds rollout attribution to `ToolInvocationAuditRecord`, `ToolInvocationAuditEntry`, `ToolInvocationAuditQuery`, `LocalToolGatewayPort`, the JDBC audit repository, `/api/tool-invocations?rolloutId=...`, `sa_tool_invocation`, startup tenant schema repair, the admin Tool Invocation Audit page, and migration `V51__tool_invocation_rollout_attribution.sql`. This closes the gap where approval/run/cost records had exact rollout attribution but tool invocation audit records could only be attributed by version/run/time window. The frontend audit surfaces also now render the backend audit contract fields directly (`toolId`, `policyDecisionId`, `resultSummary`/`errorMessage`, `startedAt`, `finishedAt`) instead of stale `toolName`/`approvalId`/`createTime`/`durationMs` fields, so rollout-filtered audit records remain visible and actionable in the admin UI.

Fresh local evidence: `.\mvnw.cmd -pl seahorse-agent-adapter-repository-jdbc,seahorse-agent-kernel,seahorse-agent-adapter-web -am "-Dtest=JdbcTenantSchemaUpgradeTests,JdbcToolInvocationAuditRepositoryAdapterTests,LocalToolGatewayPortAuditTests,KernelToolInvocationAuditQueryServiceTests,SeahorseAgentControllerTests" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed with 40/40 tests across kernel, JDBC, schema repair, and web controller coverage. Frontend evidence: `npm test -- src/services/frontendCapabilityContracts.test.ts` passed 11/11, and `npm run build` completed with only existing Browserslist/chunk-size warnings.

Follow-up cross-provider hardening: the MCP diagnostic test path now uses the rollout-aware `ToolInvocationRequest` constructor explicitly and asserts the governed request context fields (`tenantId`, `userId`, `agentIdentityId`, `toolId`, and empty `rolloutId`) before preflight/invoke. This is a regression guard for the unified Tool Gateway entry point so future request-context additions cannot silently shift MCP diagnostic audit attribution. Fresh evidence: `.\mvnw.cmd -pl seahorse-agent-adapter-mcp-http -am "-Dtest=McpServerRuntimeRegistryTests" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed 9/9, and the Tool Invocation audit focused suite above was rerun and passed 40/40 after the hardening.

### 2026-07-05 Context Pack Explain UI Contract Evidence Update

The admin Context Pack page now uses the backend context-pack contract directly instead of stale placeholder fields. The page renders `contextPackId`, `runId`, `agentId`, `versionId`, tenant, token budget, creation time, and task goal for the pack, and renders each item with source provenance, selected summary/content, estimated token cost, score, confidence, sensitivity, ACL decision id, citation JSON, and expiry time. This makes the existing Pack Explain/provenance data visible to operators from the roadmap's Context Pack productization slice.

This is a contract/UI visibility update only. It does not complete Pack Diff, tenant retention cleanup policy execution, or handoff context transfer semantics.

Fresh local evidence: `npm test -- src/services/frontendCapabilityContracts.test.ts` passed 12/12 with contract checks for `GET /api/context-packs/{}` and `GET /api/context-packs/{}/items`, plus service calls to `/api/context-packs/ctx-run-1` and `/api/context-packs/ctx-run-1/items`; `npm run build` completed with only existing Browserslist/chunk-size warnings.

### 2026-07-05 Context Pack Retention Cleanup Evidence Update

Context Pack productization now has an executable retention cleanup path for expired items. `ContextPackRetentionInboundPort` exposes `cleanupExpiredItems`, the kernel service reuses the existing Context Pack owner/admin access guard, `JdbcContextPackRepositoryAdapter` deletes only items whose `expires_at` is non-null and at or before the cutoff, and then refreshes `sa_context_pack.item_count`. The Web API exposes `POST /api/context-packs/{contextPackId}/items:cleanup-expired`, and the admin Context Pack page includes a cleanup action that refreshes the pack and item list after execution.

This is a narrow Pack Retention cleanup slice based on the existing per-item `expiresAt` contract. It does not add Pack Diff, handoff context transfer semantics, retention policy scheduling, legal hold rules, or a mutable tenant retention policy editor.

Fresh local evidence: focused Java regression passed with reactor `BUILD SUCCESS` via `.\mvnw.cmd -pl seahorse-agent-kernel,seahorse-agent-adapter-repository-jdbc,seahorse-agent-adapter-web,seahorse-agent-spring-boot-autoconfigure -am "-Dtest=KernelContextPackQueryServiceTests,KernelContextPackBuilderServiceTests,KernelAgentRunSnapshotServiceTests,JdbcContextPackRepositoryAdapterTests,SeahorseAgentControllerTests,SeahorseAgentRegistryAutoConfigurationTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`; the selected tests reported 13/13 kernel tests, 17/17 Web controller tests, and 2/2 autoconfigure tests passing, with the JDBC context-pack retention test included in the repository module run. Frontend evidence: `npm test -- src/services/frontendCapabilityContracts.test.ts` passed 12/12 with the new `POST /api/context-packs/{}/items:cleanup-expired` contract check, and `npm run build` completed with only existing Browserslist/chunk-size warnings.

### 2026-07-05 Context Pack Diff Evidence Update

Context Pack productization now has a Pack Diff API and operator surface. `ContextPackDiffInboundPort` compares two readable packs by stable context source key (`sourceType:sourceId`) instead of item id, so regenerated pack item ids do not hide source-level changes. The diff result reports added, removed, changed, and unchanged counts, plus changed-field names and left/right item payloads for changed entries. The Web API exposes `GET /api/context-packs/{contextPackId}/diff?rightContextPackId=...`, and the admin Context Pack page can query a right-side pack and render the diff summary and changed fields.

This is a Pack Diff productization slice only. It does not add visual side-by-side item rendering, handoff transfer semantics, scheduled retention policy execution, or tenant retention policy editing.

Fresh local evidence: focused Java regression passed with reactor `BUILD SUCCESS` via `.\mvnw.cmd -pl seahorse-agent-kernel,seahorse-agent-adapter-web,seahorse-agent-spring-boot-autoconfigure -am "-Dtest=KernelContextPackQueryServiceTests,SeahorseAgentControllerTests,SeahorseAgentRegistryAutoConfigurationTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`; the selected tests reported 8/8 kernel Context Pack tests, 17/17 Web controller tests, and 2/2 autoconfigure tests passing. Frontend evidence: `npm test -- src/services/frontendCapabilityContracts.test.ts` passed 12/12 with the new `GET /api/context-packs/{}/diff` contract check, and `npm run build` completed with only existing Browserslist/chunk-size warnings.

### 2026-07-05 Context Pack Handoff Reference Evidence Update

Context Pack productization now has a handoff transfer reference path. `local_agent_handoff` accepts `contextPackId`; `AgentHandoffCreateCommand`, `KernelAgentHandoffService`, and `AgentHandoff` retain that value; child A2A runs receive metadata containing `handoffId`, `parentRunId`, `contextPackId`, and the reduced context summary snapshot; and handoff audit payloads include only the context pack id plus summary lengths rather than raw input/context JSON. `JdbcAgentHandoffRepositoryAdapter` persists `context_pack_id`, startup schema has the column, and migration `V53__agent_handoff_context_pack_reference.sql` upgrades existing deployments. `SeahorseAgentHandoffController` exposes `contextPackId` in sanitized handoff responses while continuing to hide `inputSummaryJson` and `contextSummaryJson`.

The frontend Agent Inspector handoff view now uses the backend contract fields (`sourceAgentId`, `targetAgentId`, `handoffReason`, backend status values, and `contextPackId`) and the handoff service sends the required default `tenantId` query parameter for list calls. Frontend capability contracts now cover `GET /api/agent-runs/{}/handoffs`, `GET /api/agent-handoffs/{}`, and `POST /api/agent-handoffs/{}/cancel`.

This is a narrow handoff context transfer-reference slice. It does not clone context packs into child-run-owned packs, add a full handoff context diff/viewer, make transfer policies tenant-editable, or add a full-Docker multi-agent handoff E2E.

Fresh local evidence: focused Java regression passed with reactor `BUILD SUCCESS` via `.\mvnw.cmd -pl seahorse-agent-kernel,seahorse-agent-adapter-repository-jdbc,seahorse-agent-adapter-web -am "-Dtest=AgentHandoffTests,KernelAgentHandoffServiceTests,JdbcAgentHandoffRepositoryAdapterTests,SeahorseAgentHandoffControllerTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`; frontend evidence: `npm test -- src/services/frontendCapabilityContracts.test.ts` passed 13/13 and `npm run build` completed with only existing Browserslist/chunk-size warnings.

### 2026-07-05 Context Pack Handoff Full-Docker Evidence Update

The handoff transfer-reference path now has a repeatable full-Docker guard in `scripts/e2e-context-pack-handoff-smoke.ps1`. The smoke verifies login, `local_agent_handoff` Tool Gateway visibility, real tool invocation with `contextPackId`, handoff list/detail `contextPackId` exposure, sanitized detail payloads that still hide raw `inputSummaryJson` and `contextSummaryJson`, child A2A run metadata containing `handoffId`, `parentRunId`, `contextPackId`, and `contextSummaryJson`, and Tool Gateway audit recording without leaking the raw marker in `argumentsSummary`.

The first full-Docker run found a P1 deployment-upgrade gap: the long-lived PostgreSQL database lacked `sa_agent_handoff.context_pack_id`, so the current handoff repository contract failed with bad SQL grammar even though fresh init SQL and migration `V53__agent_handoff_context_pack_reference.sql` had the column. `JdbcTenantSchemaUpgrade` now idempotently adds `sa_agent_handoff.context_pack_id VARCHAR(64)` for existing deployments, with H2 regression coverage for the old table shape.

Fresh evidence: `.\mvnw.cmd -pl seahorse-agent-adapter-repository-jdbc -am "-Dtest=JdbcTenantSchemaUpgradeTests" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed 6/6; Docker proxy was corrected to the reachable local `192.168.1.9:7890` path and `docker pull docker:27-cli` succeeded; the backend image rebuilt through `docker compose -f docker-compose.full.yml build --build-arg HTTP_PROXY=http://192.168.1.9:7890 --build-arg HTTPS_PROXY=http://192.168.1.9:7890 backend` with in-image Maven `BUILD SUCCESS`; recreating backend logged `[TenantSchema] 为表 sa_agent_handoff 添加列 context_pack_id`; PostgreSQL reported `context_pack_id | character varying | YES`; and `.\scripts\e2e-context-pack-handoff-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-context-pack-handoff-smoke-live` passed 8/8 with handoff `handoff_332170575459106816`, child run `run_332170575517827072`, and context pack `context-pack-handoff-smoke-262751a7`.
### 2026-07-05 Unified GateResult Projection Evidence Update

The medium-term unified GateResult work now has its first compatibility projection. The new inbound gate contract adds `GateResult`, `GateResultItem`, and `GateResults`; the projection maps Agent `ProductionGateReport` and Run Profile `RunProfileProductionGateCheck` into a common evidence envelope with subject identity, normalized status, pass/fail boolean, blocking codes, item list, source type/id, and check time.

The Web API exposes the shared shape at `GET /api/agents/{agentId}/production-gate/gate-result` and `POST /api/run-profiles/{id}/production-gate/gate-result`. Existing object-specific endpoints remain unchanged, so current Agent and Run Profile UI/service callers can migrate gradually.
...
### 2026-07-08 Sandbox Unsupported Runtime ID Boundary P1 Fix

Real full-Docker E2E found a P1 boundary bug in the default unsupported sandbox runtime path. Invoking `sandbox_browser` against the default full-compose backend with a long marker failed before the governed response because `SandboxRuntimePort.unsupported()` derived `sessionId` from `runId` and `executionId` from that session id, overflowing the sandbox execution table's `VARCHAR(64)` id boundary. The default unsupported runtime now generates short UUID-backed `sandbox_unsupported_*` and `sandbox_exec_unsupported_*` ids, keeping the fail-closed `RUNTIME_UNSUPPORTED` behavior while avoiding value-bearing and length-unstable ids.

Fresh full-Docker evidence: the backend image rebuilt through the local `192.168.1.9:7890` proxy with in-image Maven `BUILD SUCCESS` across 28/28 modules, and the recreated default full-compose backend returned governed `RUNTIME_UNSUPPORTED` for a real `/api/tools/sandbox_browser/invoke` call with marker `seahorse-sandbox-browser-unsupported-id-boundary-final-e2e` and run `run_333060619611828224` instead of the previous PostgreSQL `value too long for type character varying(64)` failure. With the sandbox compose overlay and `ALLOWLISTED` URL policy enabled, `.\scripts\e2e-sandbox-browser-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-sandbox-browser-final-e2e -SkipBrowserImageBuild` passed 35/35, covering real browser container execution, URL allowlist egress, URL/host/session/cookie/artifact replay guards, governed downloads, audit summaries, storage, cleanup, no leftover managed containers, and zero non-terminal sandbox sessions. Follow-up full-Docker regression evidence also passed: backend smoke 20/20 under both sandbox overlay and restored default full-compose, GateResult smoke 19/19 with marker `CODX_GATE_RESULT_1783474852680`, and Run Experiment UI smoke with marker `CODX_RUN_EXP_UI_1783474863659` generated experiment `333059991062114304` and screenshot `output/playwright/artifacts/run-experiment-ui-report-CODX_RUN_EXP_UI_1783474863659.png`.

### 2026-07-08 Sandbox Browser Configured Proxy Egress P1 Evidence Update

The first browser proxy egress slice is now implemented for the container sandbox runtime. Operators can set `seahorse-agent.adapters.sandbox.container.browser-proxy-server`, or `SEAHORSE_AGENT_ADAPTERS_SANDBOX_CONTAINER_BROWSER_PROXY_SERVER` through the sandbox compose overlay, to route `sandbox_browser` URL-mode Playwright contexts through a backend-owned proxy. Tool callers cannot provide or override the proxy URL, and returned artifacts expose only `proxy.enabled` metadata rather than the proxy endpoint.

The implementation preserves existing browser URL guardrails: proxy use is limited to URL mode, Playwright request routing still enforces the allowlisted target hosts, non-browser runtimes keep the network fail-closed behavior, and `.docker.internal` proxy hosts are mapped through Docker host-gateway only for container reachability. Remaining proxy work after this first egress slice is richer product hardening: proxy authentication/rotation, operator-managed egress policy UX, richer egress audit, DNS/CIDR pinning, and long-lived browser profile governance; proxy authentication is covered by the next evidence update below.

Fresh evidence: `.\mvnw.cmd -pl seahorse-agent-adapter-sandbox-container -am "-Dtest=ContainerSandboxRuntimeAdapterTests#shouldInjectConfiguredBrowserProxyForUrlModeOnly" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed 1/1; `git diff --check` passed with only CRLF warnings; compose overlay validation passed with `SEAHORSE_AGENT_ADAPTERS_SANDBOX_CONTAINER_BROWSER_PROXY_SERVER=http://proxy.docker.internal:18082`; the full backend image rebuilt through `HTTP_PROXY=http://192.168.1.9:7890` and `HTTPS_PROXY=http://192.168.1.9:7890` with in-image Maven `BUILD SUCCESS`; `seahorse-backend` was recreated and `/actuator/health` returned `{"status":"UP"}`. Real full-Docker E2E passed through `.\scripts\e2e-sandbox-browser-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-sandbox-browser-proxy-egress-smoke-rerun -SkipBrowserImageBuild -ExpectBrowserProxy`, reporting 37/37 passing checks. The smoke verified real proxy fixture hits for both the main URL and allowlisted asset URL, confirmed downloaded result JSON included `proxy.enabled=true`, checked proxy hits did not leak cookie/session-storage secret values, downloaded governed artifacts, and confirmed no managed sandbox containers or non-terminal sandbox sessions remained. The first backend smoke run hit the real IP rate limit immediately after the long E2E; after the 60-second window expired, `.\scripts\e2e-backend-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -RuntimeProfile full-compose` passed 20/20, and the final post-rerun backend smoke also passed 20/20.

### 2026-07-08 Sandbox Browser Configured Proxy Authentication P1 Evidence Update

The configured browser proxy path now supports backend-owned proxy credentials. Operators can set `seahorse-agent.adapters.sandbox.container.browser-proxy-username` and `browser-proxy-password`, or the sandbox overlay variables `SEAHORSE_AGENT_ADAPTERS_SANDBOX_CONTAINER_BROWSER_PROXY_USERNAME` and `SEAHORSE_AGENT_ADAPTERS_SANDBOX_CONTAINER_BROWSER_PROXY_PASSWORD`, alongside `browser-proxy-server`. Tool callers still cannot provide proxy settings, `browser-proxy-server` still rejects userinfo URLs, and browser result artifacts expose only value-free `proxy.enabled` and `proxy.authenticated` booleans.

The implementation fail-closes one-sided credential configuration and requires a proxy server before credentials are accepted. The Playwright proxy object receives credentials only in URL mode, while the route allowlist and all existing URL/session guardrails remain unchanged. Remaining proxy work is now rotation, operator-managed egress policy UX, richer egress audit, DNS/CIDR pinning, and long-lived browser profile governance.

Fresh evidence: PowerShell parsing for `.\scripts\e2e-sandbox-browser-tool-smoke.ps1` passed; `.\mvnw.cmd -pl seahorse-agent-adapter-sandbox-container -am "-Dtest=ContainerSandboxRuntimeAdapterTests#shouldInjectConfiguredBrowserProxyAuthentication,ContainerSandboxRuntimeAdapterTests#shouldInjectConfiguredBrowserProxyForUrlModeOnly" "-Dsurefire.failIfNoSpecifiedTests=false" test` passed 2/2; `git diff --check` passed with only CRLF warnings; compose overlay validation passed with proxy server plus username/password; the full backend image rebuilt through the local `192.168.1.9:7890` proxy with in-image Maven `BUILD SUCCESS`; `seahorse-backend` was recreated and `/actuator/health` returned `{"status":"UP"}`. Real full-Docker E2E passed through `.\scripts\e2e-sandbox-browser-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-sandbox-browser-proxy-auth-smoke-rerun -SkipBrowserImageBuild -ExpectBrowserProxyAuth -ProxyUsername seahorse-proxy-user -ProxyPassword seahorse-proxy-password`, reporting 37/37 passing checks. The smoke verified authenticated proxy hits for both the main URL and allowlisted asset URL, confirmed downloaded result JSON included `proxy.authenticated=true`, checked JSON/HAR downloads and Tool Gateway audit summaries do not leak proxy credentials, checked proxy hits do not leak proxy credentials or cookie/session-storage secret values, and confirmed no managed sandbox containers or non-terminal sandbox sessions remained. Final backend smoke passed 20/20 against the same full-compose sandbox overlay.
