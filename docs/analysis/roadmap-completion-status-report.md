...
The sandbox-backed tool surface now includes a conservative `sandbox_file_convert` built-in tool. The kernel adapter accepts only bounded `sourceFormat=csv`, `targetFormat=json`, and inline CSV content, creates a `FILE_CONVERSION` sandbox session with network disabled, executes through `SandboxRuntimeInboundPort`, closes the session in a finally block, and returns governed artifact metadata including `scanSummary`. The built-in catalog registers the tool as `HIGH`, `EXECUTE`, and `SANDBOX`, and the existing sandbox tools feature flag disables both `sandbox_python` and `sandbox_file_convert`.

The Docker/Podman container runtime adapter now supports `FILE_CONVERSION` without exposing arbitrary code execution. For `CODE_INTERPRETER`, it preserves the existing user-script `main.py` path. For `FILE_CONVERSION`, it parses a structured conversion request, writes a generated stdlib Python converter plus `input.csv`, runs the existing no-network Python container command, and collects only `converted.json` as the prompt-governed artifact. The generated script and source input file are excluded from artifact collection, so the tool does not leak converter internals or duplicate source content into the artifact list.

Fresh evidence: focused kernel tests passed 7/7 via `.\mvnw.cmd -pl seahorse-agent-kernel "-Dtest=SandboxFileConvertToolPortAdapterTests,SandboxPythonToolPortAdapterTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`. Focused container tests passed 19/19 via `.\mvnw.cmd -pl seahorse-agent-adapter-sandbox-container -am "-Dtest=ContainerSandboxRuntimeAdapterTests,ContainerSandboxAutoConfigurationTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`, covering generated conversion script behavior, output-only artifact collection, media type classification, and unchanged fail-closed paths. Spring auto-configuration and built-in catalog tests passed 4/4 via `.\mvnw.cmd -pl seahorse-agent-spring-boot-autoconfigure -am "-Dtest=SandboxPythonToolAutoConfigurationTests,BuiltInAgentToolRegistrarTests" "-Dsurefire.failIfNoSpecifiedTests=false" test`.

Fresh full-Docker evidence: compose overlay validation passed with `SEAHORSE_AGENT_ADAPTERS_SANDBOX_CONTAINER_WORKSPACE_MOUNT_SOURCE_ROOT=/run/desktop/mnt/host/d/code/seahorse-agent/.seahorse-sandbox-compose` and `docker compose -f docker-compose.full.yml -f docker-compose.sandbox.yml config --quiet`. `docker compose -f docker-compose.full.yml -f docker-compose.sandbox.yml up -d --no-deps --build --force-recreate backend` rebuilt the backend image with an in-image Maven `BUILD SUCCESS` and restarted `seahorse-backend`. During the subsequent E2E run Docker Desktop had stopped after the environment resume, so the first script attempt timed out waiting for backend health; root cause evidence was the missing `npipe:////./pipe/dockerDesktopLinuxEngine` Docker API and stopped `com.docker.service`. Starting Docker Desktop restored engine version `29.5.2`, the compose stack recovered, and backend health returned `{"status":"UP"}`.

After the Docker engine recovered, `.\scripts\e2e-sandbox-file-convert-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-sandbox-file-convert-smoke` passed 7/7. The smoke verified `sandbox_file_convert` appears in the real built-in tool catalog as `HIGH`/`SANDBOX`, invoked it through `/api/tools/sandbox_file_convert/invoke`, verified PostgreSQL persisted `runtime_type=FILE_CONVERSION`, `profile_id=file-conversion`, closed session status `CANCELLED`, JSON artifact media type, `scan_status=CLEAN`, `sensitivity=INTERNAL`, and `scan_summary=metadata scan passed`, downloaded the converted JSON through the governed artifact endpoint, and verified the local object storage file exists with the marker. Regression evidence: `.\scripts\e2e-sandbox-python-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-file-convert-regression-smoke` passed 3/3, the active sandbox session count query returned `0`, and `docker ps -a --filter "name=seahorse-sandbox-"` returned no leftover managed sandbox containers.

Remaining roadmap work is narrowed to runtime profile policy/quota administration UX, sandbox browser automation, broader file conversion formats beyond CSV-to-JSON, virus scanning plus binary/PDF deep scanning, structured redaction-summary payloads beyond the bounded scan-summary field, stronger runtime isolation such as gVisor or Firecracker, node-pool scheduling/health beyond single-node capacity preflight, and broader A2A/cross-provider Tool Gateway hardening.
