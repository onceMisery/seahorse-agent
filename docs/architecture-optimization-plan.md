...
| 7 | P1/P2 | 架构瘦身与职责治理 | 自动配置已开始按技术域拆分；JDBC 元数据治理适配器已抽离 JSON、Schema Usage 参数组包、Schema Usage Report、quality report、canonical write、列探测、review、quarantine、backfill 协作者，quality/backfill/review/quarantine/schema usage report 旧回退已从主适配器删除，主类降至约 1225 行；端口准入规则仍待推进 | 多个独立提交 |
...
| JDBC 适配器 | JDBC 主代码约 36 个 Java 文件、31 个 `*Adapter.java`；其中 `JdbcMetadataGovernanceRepositoryAdapter.java` 已抽离 `JdbcMetadataJsonSupport`、`JdbcMetadataSchemaUsageSupport`、`JdbcMetadataSchemaUsageReportSupport`、`JdbcMetadataQualityReportSupport`、`JdbcMetadataCanonicalWriteSupport`、`JdbcMetadataColumnDetector`、`JdbcMetadataReviewSupport`、`JdbcMetadataQuarantineSupport`、`JdbcMetadataBackfillSupport` 九个协作者，quality/backfill/review/quarantine/schema usage report 旧回退已从主适配器删除，backfill 治理过滤条件已补齐到协作者，主类降至约 1225 行，但仍实现 14 个 metadata 端口 | P1，元数据治理 JDBC owner 仍偏重，但已进入可渐进拆分状态 |
| 检索编排 | `KernelMultiChannelRetrievalEngine.java` 已降至约 176 行；上下文构建与 metadata filter 编译已下沉到 `KernelSearchContextFactory`，通道发现、并发执行、timeout 降级与通道 trace 已下沉到 `KernelSearchChannelExecutor`，后处理链已下沉到 `KernelRetrievalPostProcessorChain`，观测与 metadata usage 记录已下沉到 `KernelRetrievalObservationSupport`，主类只保留 L1 串联与空结果编排 | P1，检索编排职责收敛目标基本完成 |
| 聊天主链路 | `KernelChatPipeline.java` 已降至约 98 行；历史加载、记忆激活、查询优化、改写、意图解析、引导短路和检索接线已下沉到 `KernelChatPreparationSupport`，system-only 响应、空检索兜底、RAG prompt 组装与流式返回已下沉到 `KernelChatResponseSupport`，主类只保留阶段顺序、短路判断和 trace 编排 | P1，聊天主链路阶段边界收敛目标基本完成 |
...
`JdbcMetadataGovernanceRepositoryAdapter` 当前约 1225 行，实现 schema registry、dictionary、extraction result、review queue、quarantine、canonical write、backfill、quality report、review management、quarantine management、schema management、dictionary management、extraction management、schema index status、schema usage report 等 14 个端口。当前已先把 JSON 解析、Schema Usage 参数组包、Schema Usage Report、quality report、canonical write、表结构列探测、review、quarantine、backfill 抽离到独立协作者，并删除主适配器内 quality/backfill/review/quarantine/schema usage report 旧回退实现；但主类仍承担多个事务语义、SQL 映射和运营写入职责。
...
`KernelMultiChannelRetrievalEngine` 已经通过 `SearchChannelFeature` 和 `SearchResultPostProcessorFeature` 保留了良好的扩展点。本轮已把 observation、空结果事件和 metadata usage 记录抽离到 `KernelRetrievalObservationSupport`，把通道发现、并发执行、timeout 降级和通道 trace 抽离到 `KernelSearchChannelExecutor`，把后处理器发现、排序、执行和失败跳过抽离到 `KernelRetrievalPostProcessorChain`，并把上下文构建与 metadata filter 编译抽离到 `KernelSearchContextFactory`，主类降至约 176 行；它当前只保留 L1 串联和空结果编排，检索编排职责收敛目标基本完成。
...
2. 后续拆出内部协作者：
   ...
   - `KernelRetrievalPostProcessorChain`：后处理排序、执行、失败跳过。（已完成）
   - `RetrievalTelemetryReporter`：trace、observation、empty result、metadata usage。
   - `KernelSearchContextFactory`：filter/schema/options/trace 上下文构建。（已完成）
...
`KernelChatPipeline` 已降至约 98 行，并将历史加载、记忆激活、查询优化、改写、意图解析、引导短路和检索接线抽离到 `KernelChatPreparationSupport`，将 system-only 响应、空检索兜底、RAG prompt 组装和流式返回抽离到 `KernelChatResponseSupport`。当前主类只保留固定阶段顺序、短路判断和 trace 编排。记忆写入、工具调用策略、答案安全策略继续增强时，应优先新增阶段协作者，而不是回填到主类。
