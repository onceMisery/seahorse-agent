...
| 7 | P1/P2 | 架构瘦身与职责治理 | 自动配置已开始按技术域拆分；JDBC 元数据治理适配器已抽离 JSON、Schema Usage 参数组包、Schema Usage Report、quality report、列探测、review、quarantine、backfill 协作者，quality/backfill/review/quarantine/schema usage report 旧回退已从主适配器删除，主类降至约 1268 行；端口准入规则仍待推进 | 多个独立提交 |
