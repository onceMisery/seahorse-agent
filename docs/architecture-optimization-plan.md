...
| 维度 | 当前证据 | 判断 |
|------|----------|------|
| Kernel 自动配置 | `SeahorseAgentKernelAutoConfiguration.java` 已先拆出 memory、trace、model、auth/user 与 ops/management 技术域，当前约 1007 行；仍同时装配插件、摄取、检索、聊天、知识库、元数据治理和定时任务 | P1，装配 owner 仍偏重，但已开始按职责域拆分 |
