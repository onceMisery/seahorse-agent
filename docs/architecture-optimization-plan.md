...
| 7 | P1/P2 | 架构瘦身与职责治理 | 自动配置已开始按技术域拆分；JDBC 元数据治理适配器已先抽离 JSON/Schema Usage/列探测协作者，主类降至约 2353 行；端口准入规则、聊天/检索阶段边界收敛仍待推进 | 多个独立提交 |
...
| JDBC 适配器 | JDBC 主代码约 35 个 Java 文件、30 个 `*Adapter.java`；其中 `JdbcMetadataGovernanceRepositoryAdapter.java` 已先抽离 `JdbcMetadataJsonSupport`、`JdbcMetadataSchemaUsageSupport`、`JdbcMetadataColumnDetector` 三个协作者，主类降至约 2353 行，但仍实现 14 个 metadata 端口 | P1，元数据治理 JDBC owner 仍偏重，但已进入可渐进拆分状态 |
