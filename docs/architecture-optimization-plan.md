...
| 4 | 部分完成 | OpenAI streaming 已迁移到可注入专用 executor，并由 native auto-configuration 提供默认线程池 | starter 依赖拆分、Milvus 配置化仍待规划实施 |
| 6+ | 未开始 | 无 | 清理与前端运营等 |
...
1. wrapper 占位实现清理或标注。
2. Gson 统一 Jackson。
3. `ObjectStoragePort.reliableUpload` 语义收敛。
...
如果没有区别于 `upload()` 的可靠性语义，删除 `reliableUpload()` 并修改 Local/S3 adapter。
