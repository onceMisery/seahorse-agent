...
5. `MilvusVectorAdapter` 的 HNSW 参数、内容最大长度硬编码，且 JSON 使用 Gson 而非项目主流 Jackson。
6. 自动配置类体量较大，应作为可维护性问题拆分，但不应列为 P0 阻断项。
7. RAG Trace 已补齐采样率和 TTL 清理机制，生产长期运行的表增长风险已具备最小治理闭环；后续可增强异步写入和指标聚合。

**低优先级（P2）**

8. Wrapper 占位实现、`ObjectStoragePort.upload/reliableUpload` 重复、`adapter-cache-local` 命名误导。
9. `chatStore.ts` 527 行，可按会话、消息、流式状态拆分。
10. 元数据治理后端 API 已存在，但前端缺少 Review/Quarantine/Schema/质量报表的完整管理页面。
...
该模块实际包含 `LocalSemaphoreAdapter`，实现的是 `DistributedSemaphorePort`，不是通用缓存。

**建议：** 后续可重命名为 `adapter-coordination-local`，或在模块说明中明确该模块提供本地协调能力。
...
后端已存在 Metadata Backfill、Review、Quarantine、Quality、Schema、Dictionary 等 controller，但前端 admin 页面中还没有完整的元数据治理管理页。
...
| 14 | Gson/Jackson 不统一 | `MilvusVectorAdapter` | 统一 Jackson | 减少 JSON 栈差异 |
| 15 | `ObjectStoragePort` 冗余方法 | storage port/adapters | 合并或明确 reliable 语义 | 简化接口 |
| 16 | `adapter-cache-local` 命名误导 | module name | 改名为 coordination local 或补说明 | 降低理解成本 |
| 17 | `chatStore.ts` 偏大 | frontend store | 拆 session/message/stream store | 提升前端维护性 |
| 16 | 元数据治理 UI 缺口 | frontend admin | 增加 Schema/Review/Quarantine/Quality 页面 | 完成企业治理操作闭环 |
