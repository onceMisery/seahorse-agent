...
| 闭环 | 当前实现边界 | 运行证据 |
|---|---|---|
| RAG 闭环 | 知识库入库、分块、向量检索、关键词检索、后处理、Prompt 组装、SSE 回答 | `/rag/traces/runs`、`t_rag_trace_*`、`t_knowledge_chunk` |
...
| 记忆治理 | `/memories`、`/memories/readiness`、`/memories/profile-facts`、`/memories/maintenance/run`、`/memories/governance/run` |
...
> **端点共存说明**：`/memories/maintenance/run`（SeahorseMemoryMaintenanceController，触发聚合/压缩/别名/垃圾回收等底层维护任务）与 `/memories/governance/run`（SeahorseMemoryController，触发治理策略评估、质量快照和清理建议）并存且职责不同。维护侧重数据整理，治理侧重质量管控。
...
6. `docs/zh/content/**` 中的历史 RepoWiki 文档（已归档，见 `_ARCHIVED_NOTICE.md`，不应作为事实源）。
