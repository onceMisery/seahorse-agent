...
| 文档事实源收敛 | 持续把旧 RepoWiki 文档中的部署入口、端口、默认模型、账号和 API 路径对齐当前代码。 | stale reference 扫描无旧 compose 和不存在 quick-start 引用。 | **✅ 85%** | ✅ 已修复 USER_GUIDE.md、README.md、local-embedding-model-guide.md、enterprise-mode.md 中的 `/admin/traces` 旧引用；⚠️ 归档文档(zh/content/)中残留 file:// 引用不影响事实源 |
...
| 方向 | 设计内容 | 成功证据 | 完成度 | E2E 验证状态 |
|---|---|---|---|---|
| RAG 质量评测 | 建立检索数据集、策略模板、版本对比、空召回率和命中质量指标。 | `/knowledge-base/{kb-id}/retrieval-evaluation-*` 能产出可对比报告。 | **⚠️ 95%** | ✅ Dataset 创建+评测运行成功，recall@k=1.0, MRR=1.0, NDCG@k=1.0；✅ CI 冒烟脚本 `scripts/e2e-rag-evaluation-smoke.ps1` 已创建（6 步自动化流程）；✅ 3 个策略模板可用 |
...
| 记忆质量治理 | 强化冲突检测、质量快照、review feedback、纠错 ledger、低价值记忆清理。 | `/memories/conflicts`、`/memories/quality-snapshots` 和 maintenance run 可解释。 | **⚠️ 90%** | ✅ readiness 修复后 recall_loop=READY(99)/context_injection=READY(21)/derived_index_sync=READY(21)；✅ Memory Health traceEvents=200；✅ governance/maintenance 可触发；✅ 冲突检测代码完整（`recordConflicts` 方法+SEMANTIC_KEY_CONFLICT 类型）；⚠️ 无真实冲突数据（需 semanticKey 元数据） |
| 用户画像可信度 | 画像事实增加来源、置信度、冲突状态和撤销路径；前端提供更清晰的管理入口。 | 用户能查看、修正、停用画像事实，修正影响后续召回。 | **⚠️ 95%** | ✅ 7 条 profile facts ACTIVE，API 完整返回 valueText/confidenceLevel/sourceType/version/accessCount；✅ 前端 ProfileFactsTable 增强：修复字段映射(valueText/confidenceLevel)、新增来源类型 Badge(EXPLICIT/MEMORY_AGGREGATION/explicit_user_memory)、展开详情含精确置信度和版本历史；✅ disable 端点可停用画像事实；⚠️ 前端需重新构建部署 |
...
|---|---|---|---|---|---|
| 近期设计（D1-D5） | 5 | 4 | 1 | 0 | **97%** |
| 中期 M1: RAG 质量评测 | 5 切片 | 4 | 1 | 0 | **95%** |
| 中期 M2: 入库治理 | 5 切片 | 5 | 0 | 0 | **100%** |
| 中期 M3: 记忆质量治理 | 5 切片 | 4 | 1 | 0 | **95%** |
...
|---|---|---|---|---|
| P1 | 修复 readiness API 采样限制 | D3/M3 | **✅ 已修复+部署** | listByUser() DB 级查询已上线，E2E 验证 recall_loop=READY(99) |
| P2 | M1 CI 冒烟脚本集成到 docker-compose CI | M1-Slice5 | **✅ 脚本已创建** | `scripts/e2e-rag-evaluation-smoke.ps1` 6 步自动化，需集成到 CI pipeline |
| P2 | M1 策略推广与 audit event 显式联动 | M1-Slice4 | 中 | 3 模板可用，推广 API 存在，audit 联动待验证 |
| P2 | M3 画像前端重新构建部署 | M3-Slice1 | **✅ 代码已完成** | ProfileFactsTable 字段映射+来源 Badge+展开详情已修改，TypeScript 编译通过，需 `npm run build` + Docker 部署 |
| P2 | M3 冲突数据生成验证 | M3-Slice2 | 中 | 冲突检测代码完整，需记忆捕获策略添加 semanticKey 元数据才能触发真实冲突 |
