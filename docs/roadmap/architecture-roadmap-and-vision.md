...
This completes the first P1 proxy-authentication step for browser URL egress. Remaining proxy work is now rotation, operator-managed egress policy UX, richer egress audit, DNS/CIDR pinning, long-lived browser profiles, and stronger runtime isolation.
