...
| 优先级 | 工作项 | 范围 | 验收 |
|---|---|---|---|
| P1 | 运行实验报告深化 | 已落地：Studio trace 外链、成本来源标准化；剩余：报告模板产品化 | 报告可从 runId 跳转 Studio trace，成本字段来自统一 cost 记录 |
| P1 | MCP stdio 安全治理第一阶段 | 已落地：命令 allowlist、MCP 工具 HIGH/需审批默认标记、blocked stdio stderr 诊断；剩余：审批入口、统一 Tool Gateway 执行拦截、runner 隔离 | 非 allowlist stdio 命令无法启动；高风险 MCP 工具默认进入审批/网关治理 |
