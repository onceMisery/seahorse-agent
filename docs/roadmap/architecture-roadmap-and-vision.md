...
| Sandbox Runtime 生产化 | Sandbox 策略端口、真实 Docker/Podman/gVisor runtime、MIME/内容级扫描、egress 代理、artifact 下载与详情治理 | 高风险工具和代码执行需要隔离执行面 | 高风险执行默认进入 sandbox，产物扫描通过后才能下载或注入上下文 |
