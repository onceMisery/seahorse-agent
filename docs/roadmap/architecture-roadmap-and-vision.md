...
This is one bounded create failover, not retry-until-success, queueing, fairness, session migration, execution retry, or capacity preemption. Candidate health is a placement-time snapshot; if the single failover candidate becomes stale, the request fails normally rather than refreshing the pool into an unbounded retry chain. The persisted session records only the final runtime node. A later operations-evidence slice may add a value-bounded audit event with `fromNodeId`, `toNodeId`, and `recovery=ABSENT|CLOSED`; no raw transport error or endpoint is persisted here.
