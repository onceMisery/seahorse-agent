...
当前已归档的完成基线包括：RAG、记忆与用户画像基座、Agent 治理基座、消息树、角色卡、运行方案、运行实验、RunContextSnapshot、AgentScope/Nacos A2A 基座、MCP HTTP/stdio 基座、管理后台入口可达性等。详见完成情况报告的“2026-06-22 已完成基线归档”。
...
合入 main 只能说明代码路径进入主干，不代表能力稳定。以下能力已经有代码基座，但必须补齐真实 test case 后才可进入稳定基线。

| 已合入特性 | 必补真实 test case | 稳定基线判定 |
|---|---|---|
| 消息树与分支对话 | 登录后创建会话、发送多轮消息、从历史消息 fork 分支、切换分支、刷新后恢复当前分支；校验父子关系、branch/path 字段和历史加载结果 | Playwright + 后端集成测试通过，分支不会丢消息、串上下文或破坏旧会话 |
| 角色卡 | 创建、编辑、停用角色卡，加载系统预设，把角色卡应用到聊天和 Agent run；校验提示词、约束、审批状态和上下文快照 | 角色卡能影响真实回复上下文，并在 run context snapshot 中可追溯 |
| 运行方案 | 创建运行方案，绑定角色卡、执行引擎、模型参数、记忆范围、安全策略和工具白名单；在 `/chat` 发起对话后验证会话继承 `runProfileId` | 运行方案不与用户画像混淆，执行配置进入真实 Chat/Agent Run 请求与审计快照 |
| 运行实验 | 基于同一会话和基准消息，使用两个以上运行方案创建实验；验证 trial 执行、评分、成本/trace 链接和 fork 到分支 | 实验能复现不同运行方案的输出差异，并能回到对应消息分支 |
| AgentScope / Nacos A2A | 启动本地 Nacos 与 AgentScope 配置，使用 AgentScope 运行方案触发对话；验证配置加载、A2A 注册/调用、trace、失败降级和关闭开关 | AgentScope 路径有可重复脚本或集成测试，失败时不拖垮普通聊天 |
| MCP stdio / HTTP 工具 | 启动本地 stdio MCP 示例服务，完成工具发现、参数提取、工具调用、异常返回和调用审计；覆盖 HTTP MCP 回归 | 工具调用可在真实 run 中落审计记录，高风险工具能进入审批或拒绝路径 |
| OpenAPI / A2A / 内置工具目录 | 导入 OpenAPI 连接器或使用已有工具目录项，绑定到运行方案后触发真实调用或受控失败 | 工具白名单、凭证缺失、不可用态和调用失败都有明确 UI 与审计证据 |
| 治理后台 | 真实访问各管理页面，覆盖有数据、无数据、后端不可用和权限不足场景；关键动作写入审计并能按 run/agent/tool 反查 | 不再出现 404；空态、错误态、权限态清楚，且不会影响其他后台页面 |
| Docker / 本地验证链路 | 轻量和全量部署各跑一次最小冒烟：登录、聊天、角色卡、运行方案、消息树、AgentScope 可选路径、MCP stdio 示例 | README 与路线图中的验证命令可被新环境复现 |

最低标准：每个已合入特性至少有一条正常主路径 test case、一条可复现历史问题或潜在缺陷的 test case、一条降级或错误态 test case。
...
|---|---|---|---|---|
| `docs/design/interactive-memory-conflict-resolution.md` | 交互式记忆冲突处理：`InteractiveConflictPolicy`、冲突反问模板、`memory.conflict.prompt` SSE、`resolve_memory_conflict` 工具、`POST /memories/conflicts/interactive-resolve`、聊天内冲突确认卡片、交互次数/冷却字段 | 代码搜索仅命中文档，尚未实现 | 近期 P0：记忆质量交互闭环 | 制造 PENDING 冲突，在 `/chat` 触发反问，用户选择后冲突变为 RESOLVED，记忆状态按 keep/merge/discard/update 正确变化 |
...
| `docs/design/apix-inspired-feature-evolution-roadmap.md`、`docs/design/apix-inspired-phased-implementation-design.md` | MCP stdio 治理升级：命令 allowlist、runner 隔离、风险自动标记、高风险默认禁用、审批/审计/脱敏、stderr tail 与诊断闭环 | 基础 stdio/HTTP MCP 已归档完成；统一 Tool Gateway、沙箱隔离和高风险默认策略仍需补齐 | 近期/中期：Tool Gateway 与 MCP 安全治理 | 高风险 MCP 工具默认不可直接运行；审批通过后真实调用落审计；失败时 UI 展示 stderr/诊断且不影响普通聊天 |
...
| 优先级 | 工作项 | 范围 | 验收 |
|---|---|---|---|
| P0 | 已合入 Agent 控制面真实 test case | 消息树、角色卡、运行方案、运行实验、AgentScope、MCP stdio、治理后台 | 每项至少覆盖正常、缺陷复现/回归、降级错误态 |
| P0 | 交互式记忆冲突处理 | 后端策略、SSE 事件、工具调用、前端冲突卡片、数据库交互字段 | 用户在聊天中解决冲突后，冲突状态和记忆状态正确变化 |
| P1 | 运行实验报告增强 | trial 导出、失败说明、成本/trace/分支对比、Studio trace 外链 | 实验报告可复现不同运行方案的输出差异 |
| P1 | MCP stdio 安全治理第一阶段 | 命令 allowlist、风险自动标记、高风险默认禁用、stderr 诊断、审批入口 | 高风险 stdio 工具无法绕过审批直接运行 |
...
| Sandbox Runtime 生产化 | Sandbox 策略端口、真实 Docker/Podman/gVisor runtime、MIME 扫描、egress 代理、artifact 扫描 | 高风险工具和代码执行需要隔离执行面 | 高风险执行默认进入 sandbox，产物扫描通过后才能下载或注入上下文 |
