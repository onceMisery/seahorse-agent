...
The override survives heartbeats, process restarts, and stale-registration cleanup. A returning process with the same durable node id remains drained until an administrator resumes it, while sessions already pinned to that node can still execute and close. Resume restores the latest observed admission immediately rather than manufacturing an `AVAILABLE` state. Kernel/registry remains the single admission and placement owner; this slice does not add queueing, preemption, migration, or a second scheduler. Drain/resume audit history is deferred until the override write and audit event can share a transactional outbox boundary, avoiding an API that reports failure after admission has already changed durably.
