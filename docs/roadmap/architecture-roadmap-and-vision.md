...
## 2026-07-22 Update: Tool Gateway Unified Resource Access Enforcement

The Tool Gateway resource-reference path now uses the canonical `ResourceAccessPolicyPort` through `PolicyBackedToolResourceAccessPort`. Catalog metadata supplies the trusted `resourceType` and `actionType`; callers cannot override ownership, visibility, or policy attributes. Distinct resource references are deduplicated and evaluated with AND semantics. Missing tenant/user/type, invalid references, unsupported actions, missing or failed policy decisions, and mismatched decision identity/resource fields fail closed. Downstream policy reason text is not returned to callers, and the Spring auto-configuration installs the bridge only when the canonical policy is present. The compatibility fallback is now deny-by-default, with explicit opt-in required for an alternate `ToolResourceAccessPort`.

Fresh real full-Docker evidence: the post-review backend image rebuilt all 28 modules with `BUILD SUCCESS` through `192.168.1.9:7890`, was recreated, and became healthy. `scripts/e2e-tool-resource-access-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Password admin123 -Marker seahorse-tool-resource-review-final` passed `10/10` against the live backend and PostgreSQL. The flow selected the real `tool_search` catalog entry (`LOW/READ/TOOL`), created temporary exact-resource ALLOW and DENY ACL rules for the real `admin` operator, verified ALLOW ran the actual tool and returned `sandbox_python` metadata, verified DENY returned `RESOURCE_FORBIDDEN` before execution with no resource-id leakage, verified API and direct PostgreSQL access-decision records, verified `SUCCEEDED`/`DENIED` Tool Gateway audit rows and value-free summaries, and disabled all temporary rules in cleanup. Focused regression passed `24/24` in Kernel and `3/3` in the related Spring auto-configuration combination.

This closes the roadmap's unified resource-access enforcement item at the Tool Gateway boundary. The generic backend smoke remains bounded by the known Pulsar BookKeeper ledger recovery/send failure; that unrelated infrastructure issue is retained as residual verification risk rather than folded into this slice.
...
| `docs/design/apix-inspired-feature-evolution-roadmap.md` | 发布门禁和回归评测从 Agent 扩展到 Run Profile、RAG Strategy、Model Config、Tool/Skill、Ingestion Pipeline | Agent 级 `ProductionGateReport` 已归档完成；统一 `GateResult<T>` projection 已覆盖 Agent、Run Profile、RAG Strategy、Model Config、Tool、Skill 和 Ingestion Pipeline 的当前 API，并有 full-Docker smoke；七类对象均已写入 tenant-scoped append-only 统一门禁表并有真实持久化证据；剩余为历史门禁回填、退休对象专属 gate model 与发布审批工作流替换等独立迁移决策 | 中期：统一证据模型与发布门禁 | 所有高风险对象发布前都产出 GateResult，能追溯到 evaluation、trace、audit、cost 和配置快照 |
...
Fresh evidence: `KernelSandboxRuntimeServiceTests` and `AuditEventTests` passed 75/75, including both `ABSENT` and confirmed-close `CLOSED` recovery payloads plus a failed second-runtime result that must emit no failover event. The complete 28-module production package finished with `BUILD SUCCESS`; coordinator and worker ran the identical image `sha256:83614e6c4c55643d7555b1f1496ae1137c06be5d7356bb0fb3dffc20aed0815a`. `scripts/e2e-sandbox-create-reconciliation-smoke.ps1 -VerifyAutomaticFailover -Marker seahorse-failover-audit-review-auto-20260718` passed 12/12 against real Docker, PostgreSQL, the public audit API, and the one-shot response-loss proxy. It required one `CLOSED` event with the exact node direction and attempt count in both API and database views, rejected every transport-detail marker, then executed and closed the same session on the failover node. Explicit placement passed 10/10 and produced zero failover events in both views. Independent review found and closed the failed second-runtime false-positive audit path; final review found no remaining P1/P2. The environment finished with both registrations `LIVE|AVAILABLE|0`, no reservation, no non-terminal session, no workspace, no managed child container, and no fault proxy.

## 2026-07-18 Update: Persistent Sandbox Runtime Node Drain and Resume Real E2E

Sandbox runtime-node admission now has an operator-owned persistent override instead of requiring health rows or process configuration to be mutated. The admin-only commands `POST /api/admin/sandbox/runtime/registrations/{nodeId}/drain` and `/resume` write V58's independent `sa_sandbox_runtime_node_admission_override` row with the authenticated operator and database timestamp. Heartbeats continue to persist the node's observed admission state unchanged; registration inventory exposes both observed and effective admission, while private placement endpoints and atomic capacity reservation consistently compose an operator drain as `DRAINING|false`.

The override survives heartbeats, process restarts, and stale-registration cleanup. A returning process with the same durable node id remains drained until an administrator resumes it, while sessions already pinned to that node can still execute and close. Resume restores the latest observed admission immediately rather than manufacturing an `AVAILABLE` state. Kernel/registry remains the single admission and placement owner; this slice does not add queueing, preemption, migration, or a second scheduler. The original deferred audit item is closed below by a same-PostgreSQL transaction boundary rather than a separate outbox.

Fresh evidence: focused JDBC/schema, Web, and Sandbox autoconfigure verification passed 22/22, with adapter-recreation and stale-registration-cleanup coverage proving the override remains effective after the registration row is removed and recreated. The complete 28-module production image rebuilt through `HTTP_PROXY=http://192.168.1.9:7890` and `HTTPS_PROXY=http://192.168.1.9:7890`; coordinator and worker ran the identical image `sha256:997bdffdfbaf5155413c029d749b73a939efa99e72cf468be28eb038cd0229a6`. `scripts/e2e-sandbox-runtime-node-admission-control-smoke.ps1 -Marker runtime-node-admission-review-fix-final-20260718` passed 14/14 against real Docker and PostgreSQL. It created local load plus an existing worker session, drained the worker, observed a later real heartbeat without losing the override, restarted the worker and required a new owner UUID to re-register as observed `AVAILABLE` but effective `DRAINING`, executed and closed the pre-restart session, routed automatic placement away, rejected explicit placement with `RUNTIME_NODE_DRAINING`, resumed the worker, and automatically selected it again as the less-loaded node. The existing remote-runtime regression then passed 20/20. Independent review found and closed missing restart/registration-cleanup evidence; re-review found no remaining P1/P2. The environment finished with both registrations `LIVE|AVAILABLE|0`, `draining=false`, no reservation, no non-terminal session, no workspace, and no managed child container.

## 2026-07-18 Update: Atomic Sandbox Runtime Node Admission Audit

Drain/resume now carries a kernel-generated audit identity and tenant through the inbound port. The JDBC registry owns one transaction that locks the durable node/override, writes the admission override, and appends `SANDBOX_RUNTIME_NODE_ADMISSION_CHANGED` to `sa_audit_event` through the same `DataSource`; an audit failure rolls back the override. Every operator command, including an idempotent command, produces one event. The event is correlated to `SANDBOX_RUNTIME_NODE/{nodeId}`, records `USER/{operatorId}` and the effective tenant, and stores only `{"draining":true|false}`. A retained override is the durable identity of a previously registered node, so stale-registration cleanup does not make that node unknown; a node with neither a registration nor an override remains rejected without an audit row.

Custom non-JDBC audit repositories fail closed for the atomic admission command. This preserves the single transactional owner and avoids claiming atomicity across an unknown persistence boundary. The admin controller passes the authenticated tenant, while the existing observed/effective admission, heartbeat, restart, placement, pinned-session, and cleanup contracts remain unchanged.

Fresh evidence: focused JDBC registry, Web, and registry autoconfiguration verification passed 10/10, including audit row shape, timestamp correlation, audit-save rollback, controller tenant propagation, default JDBC wiring, and custom-audit fail-closed behavior. The final complete production package was deployed as the same application artifact to coordinator and worker. `scripts/e2e-sandbox-runtime-node-admission-control-smoke.ps1 -Marker runtime-node-admission-atomic-audit-final-20260718` passed 14/14 against real Docker and PostgreSQL, checking the API and database audit views for the idempotent resume, drain, and resume commands. The existing `scripts/e2e-sandbox-remote-runtime-smoke.ps1 -Marker runtime-node-admission-audit-regression-20260718` passed 20/20. The environment finished with both registrations `LIVE|AVAILABLE|0`, `draining=false`, zero capacity reservations, zero non-terminal sessions, no workspaces, and no managed child containers. Independent review found no P1/P2 issues; the remaining P3 documentation and durable-node semantic ambiguity were closed in this update.

## 2026-07-19 Update: Sandbox Runtime Node Maintenance Readiness

Administrators can now read `GET /api/admin/sandbox/runtime/registrations/{nodeId}/maintenance-status` as a PostgreSQL-backed safety decision rather than inferring shutdown readiness from heartbeat load. One database statement reports the durable drain state, exact non-terminal persisted-session count, unexpired capacity-reservation count, create-operation tracking capability, all unfinished create operations for the node, drain timestamp, ten-minute stabilization deadline, and checked timestamp. `maintenanceReady=true` requires operator drain, zero sessions, zero reservations, a tracking-capable live owner or no live registration, zero unfinished creates, and an elapsed stabilization window. A retained override remains a known durable node after stale registration cleanup; a node with neither registration nor override fails closed.

V59 adds owner-bound worker capability and no-TTL create-operation records. New workers advertise capability on heartbeat. Remote create registers after signed owner validation and releases in `finally`; Kernel wraps local create with the same begin/end lifecycle. Maintenance counts operations from every owner, so owner takeover, lease expiration, and stale-registration cleanup cannot hide a still-running old create. An operation row is removed only by the completing create path; a crashed process can therefore require explicit future recovery work instead of being guessed complete. All registered nodes, including unlimited-capacity nodes, also retain the existing five-minute coordinator reservation around create. Durable drain is checked under the node/override lock even before the first heartbeat, closing the override-only local admission gap.

The ten-minute drain stabilization window remains a separate mixed-coordinator compatibility guard. Drain first blocks all later old and new admissions through the existing node-row plus override lock. The window then outlives the configured remote request timeout and the maximum operation lease, covering requests admitted by an older coordinator that did not reserve unlimited capacity. Worker capability is used only to prove the target worker tracks its own create execution; it is not treated as evidence about coordinator version. This slice does not add migration, preemption, queueing, force cleanup, or a second scheduler.

Fresh evidence: focused Kernel, JDBC/schema, Web, and transport verification passed 101/101. The complete 28-module production package finished with `BUILD SUCCESS` and the identical executable jar was deployed to coordinator and worker. `scripts/e2e-sandbox-runtime-node-admission-control-smoke.ps1 -Marker maint-ready-final-0719` passed 14/14 against real Docker and PostgreSQL, including heartbeat/restart drain persistence, exact session and reservation counts, the stabilization boundary, an injected current-owner in-flight create that forced readiness false, routing, resume, and cleanup. `scripts/e2e-sandbox-remote-runtime-smoke.ps1 -Marker maint-ready-final-reg-0719` passed 20/20 across signed remote create, local and remote placement, execution, artifact transfer, drain routing, orphan recovery, and cleanup. Independent review found and closed the unbounded create, override-only admission, local tracking, and owner-takeover gaps; final review found no remaining P1/P2. The environment finished with both registrations `LIVE|AVAILABLE|0`, `draining=false`, zero reservations, zero unfinished create operations, zero non-terminal sessions, no workspaces, and no managed child containers.

## 2026-07-19 Update: Sandbox Runtime Node Operations Console

The admin Sandbox Runtime governance panel now exposes the shared runtime-node registry as an operations surface instead of leaving registration and maintenance contracts API-only. Each registered node shows its LIVE/STALE inventory state, observed admission and session load, and effective admission after the durable operator override. Administrators can drain or resume an individual node from an icon action. A drained row expands the PostgreSQL-backed maintenance decision with exact persisted-session, reservation, create-tracking, in-flight-create, stabilization, deadline, and ready/not-ready state. The existing local runtime health projection remains separate, so the UI does not create a second scheduler or merge node-local health ownership into the shared registry.

Runtime governance refreshes registration, maintenance, scanner, profile, and health projections as one readback. Maintenance status is requested only for drained nodes. A command is not reported as synchronized until the complete readback succeeds; a failed refresh clears the governance projection and retains an explicit error instead of presenting stale `AVAILABLE` or `READY` state. Node ids are encoded in every maintenance command path, and each node keeps an independent action state. This slice does not add force cleanup, session migration, preemption, queueing, or placement policy.

Fresh evidence: frontend service contracts and endpoint coverage passed 19/19, `node --check scripts/e2e-sandbox-tool-quota-page-smoke.mjs` passed, and the production frontend build completed successfully both locally and inside the Docker image. The rebuilt `seahorse-frontend` was deployed against the existing full Docker environment. `scripts/e2e-sandbox-tool-quota-page-smoke.ps1 -BaseUrl http://127.0.0.1 -Password admin123 -Marker seahorse-runtime-node-governance-ui-review-fix-0719` passed against the real frontend, backend, and PostgreSQL data. It required `sandbox-node-b` to begin `LIVE|AVAILABLE`, drained it through the page, read back effective `DRAINING`, verified maintenance `NOT READY` with zero sessions, reservations, and in-flight creates plus available create tracking and a pending stabilization window, resumed it through the page, and read back `AVAILABLE`. The script retains a finally-path API resume to prevent failed runs from leaving a node drained. Independent review found and closed stale governance projection after a failed refresh; final re-review found no remaining P1/P2. The environment finished with both registrations `LIVE|AVAILABLE|0`, zero draining overrides, reservations, unfinished create operations, and non-terminal sessions, with no managed sandbox child containers.

## 2026-07-19 Update: Sandbox OCI Runtime Capability Health

The optional OCI runtime configuration is now a verified runtime capability rather than a display-only setting. Docker health uses a bounded `docker info` template to check registered runtime names; Podman health checks the current `Host.OCIRuntime.Name`. A configured runtime that cannot be verified changes runtime health to `UNAVAILABLE`, causes node admission to fail closed, and emits only a value-free failure reason. An empty configuration retains the engine default behavior. Runtime health and node health expose `ociRuntimeAvailable`, while Sandbox Operations and the real smoke scripts show and assert `runc / AVAILABLE`.

This closes the operational gap between selecting an isolation class and proving that the daemon can honor it. The probe exposes no runtime paths, feature metadata, host details, or full runtime registry. It does not claim that gVisor or Firecracker is installed; those remain stronger-isolation deployment work, alongside broader node-pool scheduling and health.

Fresh evidence: focused adapter runtime capability and fail-closed checks passed 3/3, `KernelSandboxRuntimeServiceTests` passed 73/73, `SeahorseSandboxControllerTests` passed 2/2, the frontend capability contract passed 17/17, the production frontend build and script syntax checks passed, and the complete backend/frontend Docker image build finished successfully through the local `192.168.1.9:7890` proxy. The sandbox overlay recreated a healthy backend with `ociRuntime=runc` and `ociRuntimeAvailable=true`. Real full-Docker `scripts/e2e-sandbox-python-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -ExpectedOciRuntime runc` passed 8/8, including a real child-container execution. Real packaged-browser `scripts/e2e-sandbox-tool-quota-page-smoke.ps1 -BaseUrl http://127.0.0.1 -ExpectedOciRuntime runc` passed and asserted the rendered `runc / AVAILABLE` value while exercising drain/resume. Final PostgreSQL inspection showed both nodes `HEALTHY / AVAILABLE / active=0`, zero draining overrides, reservations, unfinished create operations, and non-terminal sessions, with no managed sandbox child containers.

## 2026-07-19 Update: Sandbox OCI Runtime Fail-Closed Deployment Gate

The OCI runtime capability now has a repeatable full-Docker degradation gate through `scripts/e2e-sandbox-oci-runtime-fail-closed-smoke.ps1`. The smoke refuses to inject a deployment fault while non-terminal sessions, reservations, in-flight creates, or managed sandbox child containers exist. Before recreation it resolves the compose model and compares every compose-owned backend environment value with the running container, so the test cannot silently reset unrelated deployment configuration.

The gate proves that the local Docker daemon does not register `runsc`, temporarily recreates only the backend with `oci-runtime=runsc`, and requires runtime health plus local node admission to report `UNAVAILABLE` with a value-free reason. It creates a real Agent Run and explicitly placed sandbox session, then requires `FAILED|RUNTIME_NODE_UNAVAILABLE`, no node assignment, no workspace or child container, and no capacity/create-operation residue. A `finally` path recreates the backend with the captured original runtime and now requires the restored capability to be `HEALTHY` and available.

Fresh evidence: `scripts/e2e-sandbox-oci-runtime-fail-closed-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Marker oci-runtime-fail-closed-env-guard-20260719` passed 8/8 against the real Docker daemon, backend API, PostgreSQL, and workspace mount while switching `runc -> runsc -> runc`. After restoration, `scripts/e2e-sandbox-python-tool-smoke.ps1 -BaseUrl http://127.0.0.1:9090 -Marker oci-runtime-env-guard-restored-20260719 -ExpectedOciRuntime runc` passed 8/8 and executed a real child container. This verifies the missing-runtime fail-closed boundary and recovery procedure; it does not claim that gVisor or Firecracker is installed.

## 2026-07-19 Update: Optional OpenTelemetry Agent Trace Export

The Micrometer observation adapter now has an optional OpenTelemetry OTLP bridge. The explicit Seahorse RAG trace lifecycle is mirrored through a `TraceTelemetryPort`, preserving cross-thread parentage by logical run/node ids instead of relying on a thread-local tracing scope. Agent runs export `agent.run -> agent.step -> model.call/tool.call` spans, while the existing Micrometer timer/counter behavior remains unchanged when tracing is disabled. Span attributes are limited to operational tenant, agent, executor, and run dimensions, bounded in length, and error messages pass through credential redaction before export.

The full compose adds a lightweight Jaeger OTLP receiver for verification. Tracing is opt-in through `SEAHORSE_OBSERVABILITY_TRACING_ENABLED`; the default remains disabled and the endpoint/sampling rate are configurable through the matching `SEAHORSE_OBSERVABILITY_TRACING_*` variables. The receiver is not required for normal metrics-only deployments.

Fresh evidence: the complete backend package finished with `BUILD SUCCESS`; Jaeger received a real Agent chat trace from the full Docker backend with 7 spans and the hierarchy `agent.run -> agent.step -> model.call -> tool.call`, including `seahorse.agent.id=legacy-react-agent`, `seahorse.executor.engine=kernel`, `seahorse.tenant.id=default`, and the persisted `seahorse.run.id`. The real chat returned HTTP 200 and `[DONE]`, and the governed knowledge-search tool completed through the Tool Gateway. A separate real full-Docker run with tracing disabled returned HTTP 200 and `[DONE]` while Jaeger `agent.run` count remained unchanged (`4 -> 4`). The sandbox Python path independently failed closed with `RUNTIME_NODE_UNAVAILABLE` because no runtime node was available; this infrastructure fault did not affect the tracing evidence or leak raw credentials into spans.

## 2026-07-19 Update: Unified GateResult Persistence Foundation

Unified GateResult now has a tenant-scoped append-only persistence owner. `V60__unified_gate_result.sql` introduces `sa_gate_result` with subject identity, status/pass decision, blocking codes, item evidence, checked time, source identity, and creation time. The JDBC adapter resolves tenant ownership exclusively from `TenantContext`, normalizes subject type at the persistence boundary, appends every decision, and returns the latest row by checked time and insertion order. `GET /api/gate-results/{subjectType}/{subjectId}` exposes that durable latest projection through a kernel inbound service.

This first persistence slice connects real Agent production-gate generation and Run Profile production-gate checks. It preserves the existing Agent `ProductionGateReport` table, object-specific APIs, and gate semantics; no backfill or destructive retirement is performed. Tool, Skill, RAG Strategy, Model Config, and Ingestion Pipeline still use their existing live projections and remain the next persistence expansion boundary.

Fresh evidence: the complete 28-module backend package finished with `BUILD SUCCESS`. Backend image `sha256:41564bf025c4fefdf51c4b6d0d0cee451f4efac63c0d0504665845700ca7662e` was built through `192.168.1.9:7890`, deployed, and reached healthy. `scripts/e2e-gate-result-smoke.ps1` passed 24/24 against the real full-Docker API and PostgreSQL, including Agent/Run Profile append-only rows, unified latest readback, complete item/source/blocking-code JSON, and a future-dated cross-tenant sentinel that remained invisible to the default tenant. `scripts/e2e-backend-smoke.ps1 -RuntimeProfile full-compose` passed 20/20 across health, auth, real knowledge CRUD/chunking, RAG SSE/trace, memory/profile, catalogs, audit, metadata governance, and SRE health. Docker cleanup reclaimed approximately 18.66 GB from old BuildKit cache before the final image build and another 4.46 GB afterward without pruning data volumes; the final full-Docker services remained healthy.

## 2026-07-19 Update: Unified GateResult Persistence Completion

The append-only persistence owner now covers all seven unified GateResult subject types. Tool, Skill, Model Config, Ingestion Pipeline, and RAG Strategy gate-result routes explicitly record their projected decision through `GateResultInboundPort`, joining the existing Agent and Run Profile write paths. Ordinary catalog, configuration, pipeline, and comparison reads remain side-effect free. Existing direct controller constructors retain passthrough behavior for compatibility, while the Spring runtime injects the durable recorder. For explicit cross-tenant Skill and Model Config administration, the recorder scopes the write to the source object's tenant and restores the authenticated request context in `finally`.
