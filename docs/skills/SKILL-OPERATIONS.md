...
内置技能存放在 `seahorse-agent-spring-boot-starter/src/main/resources/skills/public/` 下，每个子目录为一个技能：
...
| `allowed_tools` | 否 | string[] | 建议工具列表（仅 advisory，不扩展实际权限） |
...
| 技能数 > 3 或总内容 > 12000 字符 | `METADATA_ONLY` | 仅注入元数据，模型可通过 `load_skill` 按需加载 |
...
3. **allowed_tools 仅 advisory**：不扩展实际的工具授权（`allowedToolIds`）
...
1. 在 `seahorse-agent-spring-boot-starter/src/main/resources/skills/public/` 下创建新目录
