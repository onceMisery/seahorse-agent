...
### 统一门禁结果接口（GateResult）
统一门禁结果把 Agent、运行方案、工具、Skill、模型配置、入库 Pipeline 和 RAG 策略七类对象的发布前判定收敛成同一份证据模型，并以租户隔离的 append-only 表持久化。

- 查询最新门禁结果
  - 方法与路径：GET /api/gate-results/{subjectType}/{subjectId}
  - 说明：返回该对象最近一次门禁判定；不存在时返回资源未找到
  - 响应字段：subjectType、subjectId、status（PASS/FAIL/WARN 等）、passed、blockingCodes、items、checkedAt、sourceType、sourceId
- 查询门禁历史
  - 方法与路径：GET /api/gate-results/{subjectType}/{subjectId}/history
  - 查询参数：limit（可选，默认 20，服务端上限 100）
  - 说明：按 checkedAt 倒序返回该对象的历史门禁记录，用于审计追溯与回归对比
  - 响应：门禁结果对象列表，字段同上
...
章节来源
- [SeahorseGateResultController.java](file://seahorse-agent-adapter-web/src/main/java/com/miracle/ai/seahorse/agent/adapters/web/SeahorseGateResultController.java)
