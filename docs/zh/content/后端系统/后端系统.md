...
- 自动装配：`SeahorseAgentKernelAutoConfiguration` 现在主要作为聚合入口，通过 `@Import` 组合 auth、chat、knowledge、memory、metadata、model、ops、plugin、retrieval、trace 等子自动配置
- starter 分层：`seahorse-agent-spring-boot-starter` 现在承担精简自动装配入口，并将 Milvus、PgVector、Redis、S3、Pulsar、Tika、OpenAI-compatible、ES、Lucene、Micrometer 等重型适配器依赖改为 `optional`；`seahorse-agent-spring-boot-starter-all` 提供全量官方适配器聚合入口
- 插件装配：`SeahorseAgentKernelPluginAutoConfiguration` 负责 Feature 激活上下文、扩展注册表、健康聚合器，以及摄取/检索默认 Feature 注册
