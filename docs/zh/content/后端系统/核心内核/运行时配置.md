...
- [application.properties（starter）](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties)
...
- [AgentKernelProperties.java:29-77](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentKernelProperties.java#L29-L77)
- [AgentAdapterProperties.java:29-58](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentAdapterProperties.java#L29-L58)
- [AgentPluginProperties.java:30-65](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentPluginProperties.java#L30-L65)
- [SeahorseAgentKernelAutoConfiguration.java:181-188](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/SeahorseAgentKernelAutoConfiguration.java#L181-L188)
...
- 自动配置拆分：主自动配置类只负责属性启用条件、`@Import` 聚合和少量跨域强依赖 Bean；Feature 注册与默认插件装配已迁移到 `SeahorseAgentKernelPluginAutoConfiguration`
...
- 条件注解：
  ...
  - 当未显式配置时，默认启用内核装配
- 作用：在满足条件时聚合各技术域自动配置，并保留少量跨域 Bean 的兼容装配

#### 当前拆分结构
- `SeahorseAgentKernelAutoConfiguration`：负责启用条件、配置属性绑定、子自动配置聚合，以及少量跨域 Bean
- `SeahorseAgentKernelPluginAutoConfiguration`：负责 `FeatureActivationContext`、`ExtensionRegistry`、`FeatureHealthAggregator` 以及默认 ingestion/retrieval Feature 注册
- 其他子自动配置：分别承载 auth、chat、document refresh、keyword、knowledge、memory、metadata、model、ops、retrieval、trace 等技术域
...
**章节来源**
- [SeahorseAgentKernelAutoConfiguration.java:181-188](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/SeahorseAgentKernelAutoConfiguration.java#L181-L188)
- [SeahorseAgentKernelPluginAutoConfiguration.java](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/SeahorseAgentKernelPluginAutoConfiguration.java)
...
**章节来源**
- [application.properties（引导）:1-4](file://seahorse-agent-bootstrap/src/main/resources/application.properties#L1-L4)
- [application.properties（starter）:1](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties#L1)
...
- SeahorseAgentKernelPluginAutoConfiguration 依赖 AgentPluginProperties 构建 Feature 激活上下文，并负责默认 Feature 注册
...
- [AgentKernelProperties.java:33-34](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentKernelProperties.java#L33-L34)
- [AgentKernelProperties.java:61-75](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentKernelProperties.java#L61-L75)
...
- 内核未装配
  ...
  - 参考：[SeahorseAgentKernelAutoConfiguration.java:181-188](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/SeahorseAgentKernelAutoConfiguration.java#L181-L188)
- 扩展缺失导致失败
  - 症状：启动失败或功能不可用
...
- [application.properties（引导）:1-4](file://seahorse-agent-bootstrap/src/main/resources/application.properties#L1-L4)
- [application.properties（starter）:1](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties#L1)
- [AgentKernelProperties.java:40-75](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/config/AgentKernelProperties.java#L40-L75)
