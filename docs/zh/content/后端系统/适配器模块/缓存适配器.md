...
## 简介
本文件面向“缓存适配器”的技术文档，系统阐述两类适配器：本地内存适配器与 Redis 适配器。`seahorse-agent-adapter-cache-local` 保留该模块名是为了兼容既有 Maven 坐标，但它实际提供“本地内存缓存 + 单 JVM 协调”能力，不适合作为多节点分布式协调实现。内容涵盖：
- 缓存接口的统一抽象与实现边界
- 分布式锁与信号量的实现机制与差异
- 缓存配置要点、过期策略与内存管理
- 性能优化建议、故障转移方案与监控指标
