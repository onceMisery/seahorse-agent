...
- 前端仪表盘：性能阈值与可视化
- 性能基准文档：`docs/performance/current-baseline.md` 登记真实压测、追踪与指标导出
...
章节来源
- [seahorse_init.sql:420-436](file://resources/database/seahorse_init.sql#L420-L436)
...
- [DashboardPage.tsx:103-1189](file://frontend/src/pages/admin/dashboard/DashboardPage.tsx#L103-L1189)
- [SeahorseAgentNativeAdapterAutoConfiguration.java:370-575](file://seahorse-agent-spring-boot-autoconfigure/src/main/java/com/miracle/ai/seahorse/agent/adapters/spring/SeahorseAgentNativeAdapterAutoConfiguration.java#L370-L575)
...
### 数据库模式与索引设计
- 向量表与索引
  - 向量维度由 Embedding 模型解析；当前全量默认 `nomic-embed-text` 为 768 维，使用 HNSW + 余弦距离
...
### 向量维度与模型选择
- 当前向量维度由 Embedding 模型解析；全量默认 `nomic-embed-text` 为 768 维，适配器与 Milvus 属性均要求正整数维度
- 建议
  - 与 Embedding 模型一致，避免运行时校验失败
  - 如需降低存储与计算成本，可在模型与硬件允许前提下调小维度，但需回归验证检索质量

章节来源
- [seahorse_init.sql:426](file://resources/database/seahorse_init.sql#L426)
...
章节来源
- [seahorse_init.sql:43-43](file://resources/database/seahorse_init.sql#L43-L43)
- [seahorse_init.sql:79-80](file://resources/database/seahorse_init.sql#L79-L80)
- [seahorse_init.sql:96-98](file://resources/database/seahorse_init.sql#L96-L98)
- [seahorse_init.sql:109](file://resources/database/seahorse_init.sql#L109)
- [seahorse_init.sql:155](file://resources/database/seahorse_init.sql#L155)
- [seahorse_init.sql:196](file://resources/database/seahorse_init.sql#L196)
- [seahorse_init.sql:240-241](file://resources/database/seahorse_init.sql#L240-L241)
- [seahorse_init.sql:429-430](file://resources/database/seahorse_init.sql#L429-L430)
- [seahorse_init.sql:37-39](file://resources/database/seahorse_init.sql#L37-L39)
- [seahorse_init.sql:58-60](file://resources/database/seahorse_init.sql#L58-L60)
...
章节来源
- [seahorse_init.sql:429-430](file://resources/database/seahorse_init.sql#L429-L430)
- [seahorse_init.sql:105-115](file://resources/database/seahorse_init.sql#L105-L115)

### 实际性能测试与优化对比
- 基准与 after 场景
  - 以 `docs/performance/current-baseline.md` 登记多场景压测基线与真实导出，用于回归对比
  - 建议在每次优化后，使用相同场景与负载进行对比，控制回归幅度
- 指标项
  - 首 token 延迟、总聊天时延、检索总时延、多通道检索、MCP 编排、内存加载、模型路由、文档摄取

章节来源
- [rag-baseline.json:1-52](file://docs/USER_GUIDE.md#L1-L52)
- [current-baseline.md](file://docs/performance/current-baseline.md)
- [current-baseline.md](file://docs/performance/current-baseline.md)
- [current-baseline.md](file://docs/performance/current-baseline.md)
