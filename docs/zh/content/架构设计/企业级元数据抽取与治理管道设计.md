...
- 为 Elasticsearch、PostgreSQL、Lucene Embedded、Milvus/PGVector 提供稳定、规范、可解释的元数据；OpenSearch 后续按同一关键词索引端口补充。
...
| 索引写入        | `IndexerNodeFeature` 写关系库和向量库                                     | 联动 PostgreSQL JSONB、Elasticsearch、Lucene Embedded 和向量库过滤字段          |
...
flowchart TD
    ...
    K --> ES["Elasticsearch / Lucene Embedded<br/>BM25 + metadata fields"]
...
| L3 适配器     | JDBC、YAML、Elasticsearch、Lucene、PostgreSQL、OpenAI Compatible                                                                                  | 提供可替换实现                 |
...
- Lucene Embedded 已使用相同 `KeywordSearchPort` / `KeywordIndexPort` 契约落地为低优先级嵌入式实现，通过显式配置启用，适合测试、单机和私有化极简部署。
- OpenSearch 继续使用相同端口契约作为后续规划，仅在企业已有 OpenSearch 基础设施时引入。
...
- 若部署侧显式启用 Lucene Embedded，accepted metadata 会随关键词索引快照写入 Lucene 过滤字段和 `metadata_json`；Quarantine 或未通过复核的数据不得进入该索引。
...
- 质量报表查询已支持按 `schemaVersion` 和 `extractorVersion` 过滤抽取快照，`metadata.quality.report.generated` 观测事件也会携带这两个筛选维度，便于对比不同 Schema、规则抽取器或 LLM prompt/extractor 版本的治理效果。
...
- 历史回填服务已记录 `metadata.backfill.job.created`、`metadata.backfill.job.paused`、`metadata.backfill.job.resumed`、`metadata.backfill.job.cancelled` 以及 `metadata.backfill.batch.success/failure` 观测事件，包含任务状态、页码、批量计数、失败数、Schema/Extractor 版本和暂停原因，不携带文档内容或字段值。
...
- Quarantine 数据不能进入 Elasticsearch、Lucene Embedded、向量库和 RAG prompt。
...
2. Elasticsearch mapping 由 Schema 驱动更新，生产默认异步写入；Lucene Embedded 作为显式启用的嵌入式关键词索引实现，消费同一份 accepted metadata。
...
- 能把 accepted metadata 写入 PostgreSQL、Elasticsearch 和向量库；显式启用 Lucene Embedded 时，同步写入 Lucene 关键词索引。
- 未注册字段不能参与元数据过滤。
...
- DDL 每个新增表和新增字段都有注释。
- Elasticsearch、Lucene Embedded 和后续 OpenSearch 只作为 `KeywordIndexPort` / `KeywordSearchPort` 的可替换实现；默认优先 Elasticsearch，PostgreSQL 作为治理存储和轻量 fallback，Lucene 仅显式启用。
...
| 索引测试   | Elasticsearch mapping、Lucene Embedded 适配器契约、PostgreSQL JSONB、向量 metadata 下推   |
...
- 存储侧写入 PostgreSQL `metadata_json`，并同步 Elasticsearch / Lucene Embedded / 向量库必要字段；OpenSearch 后续按同一端口契约补充。
