...
- 元数据抽取和校验节点已记录 `metadata.extraction.completed`、`metadata.validation.completed` 观测事件，包含 tenant、kb、schemaVersion、extractorVersion、耗时、候选/问题数量和校验决策，作为后续指标适配器落库或上报的统一证据。
