...
### 8.1 记忆系统近期演进（P0/P1 实施项）

记忆系统的核心读路径（四层记忆 / 混合召回 / 别名合并 / 压缩）已经完成，下一阶段聚焦**衰减算法、质量评估和冲突治理**的深化：

| 优先级    | 实施项                 | 说明                                                                                 |
| ------ | ------------------- | ---------------------------------------------------------------------------------- |
| **P0** | 仿生衰减算法              | 基于 `e^(-λt)` 的时间衰减模型，接入 `ShortTermMemoryMaintenancePort`，让记忆像人脑一样"用进废退"            |
| **P0** | 质量评估器               | 五维度评估体系：新鲜度（freshness）、相关性（relevance）、置信度（confidence）、使用频次（usage）、一致性（consistency） |
| **P1** | 冲突检测与修复             | 基于 `t_memory_conflict_log` 表的自动化冲突检测服务，配合交互式冲突处理机制形成闭环                             |
| **P1** | 质量报表                | `t_memory_quality_snapshot` 表 + 定时快照服务，让记忆质量可量化、可追踪                                |
| **P2** | MemoryVectorPort 闭环 | 接入 Milvus 实现语义记忆的向量检索，提升跨会话召回精度                                                    |
| **P2** | 多级摘要策略              | L2 会话级摘要 → L3 跨会话主题摘要 → L4 用户画像摘要，构建层次化的记忆压缩体系                                     |

### 8.2 远期规划（3-6 个月）

记忆系统将与平台其他能力深度联动，形成更完整的企业级认知基础设施：

| 方向           | 规划内容                                                        | 说明                                                                |
| ------------ | ----------------------------------------------------------- | ----------------------------------------------------------------- |
| **企业数据边界联动** | 统一资源标识（`resourceType/resourceId/action/subject`）            | 所有模块（Agent/Tool/Knowledge/Memory）复用统一的资源身份模型，记忆访问受 ACL 和 Quota 约束 |
| **执行前决策联动**  | Agent run / Tool / Context Pack / Sandbox 执行前评估 ACL + Quota | 记忆召回结果在进入对话前经过权限和配额校验，确保企业数据安全                                    |
| **执行后记账**    | Cost Usage + Audit Event + Access Decision Log 统一写入         | 每次记忆操作（读取/写入/治理）都产生可审计的事件记录                                       |
| **可解释记忆网络**  | Memory Lineage 查询 + 前端记忆图谱                                  | "为什么你记得这个？"——展示记忆的来源、派生关系和影响范围                                    |
| **自适应知识运营**  | RAG Trace 聚合 + 修复建议引擎                                       | 空召回/高延迟/差评答案自动聚合，触发知识库修复建议                                        |
| **人机协作控制面**  | 统一操作事件模型 + "待我处理"视图                                         | 记忆冲突、审批、通知统一入口，按风险和 SLA 排序                                        |

### 8.3 项目地址

Seahorse Agent 是一个开源项目，欢迎 Star、Fork 和贡献代码：

> **GitHub**: [https://github.com/onceMisery/seahorse-agent](https://github.com/onceMisery/seahorse-agent)

**快速开始**：

```bash
# 克隆项目
git clone https://github.com/onceMisery/seahorse-agent.git
cd seahorse-agent

# 轻量部署（前端 + 登录 + 基础 API 冒烟）
docker-compose up -d

# 全量部署（Milvus + Ollama + Redis + ES + Pulsar + MinIO + Prometheus + Grafana）
docker-compose -f docker-compose.full.yml up -d
```

**技术栈**：Java 21 + Spring Boot 3.5 + React 18 + TypeScript + PostgreSQL + Milvus + Redis + Pulsar
