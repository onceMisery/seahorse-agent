...
public record QueryOptimizationResult(
        ...
        List<String> expandedTerms,   // 检索侧消费
...
- `expandedTerms`：检索侧消费，不应直接污染用户问题
...
- 查询状态可观测：debug log 中能看到 protectedTerms / expandedTerms，keyword 检索能消费 expandedTerms
...
当前已有 JDBC 实现：`JdbcQueryTermExpansionAdapter` 读取启用且未删除的 `t_query_term_mapping` 规则，支持 exact、prefix/fuzzy 和可选 regex，并通过 `SeahorseAgentOpsRepositoryAutoConfiguration` 默认装配。`RuleBasedQueryOptimizerPort` 产出的 `expandedTerms` 会经 `KernelRetrievalEngine`、`RetrievalOptions.channelSettings` 和 `SearchContext.metadata` 传入 `KeywordSearchChannelFeature`，用于扩展 keyword 查询；用户原始问题、rewrite 输入和向量查询不会被直接拼接扩展词。没有 JDBC adapter 或查询失败时仍保留 noop/fail-open 降级行为。
...
- id: VARCHAR(64), 主键
- user_id: VARCHAR(64)
- memory_embedding: FLOAT_VECTOR, dim 由 embedding 模型解析（当前全量默认 nomic-embed-text 为 768）
