...
本文基于 `docs/zh/content/架构设计/混合检索.txt` 和《[混合检索与重排详细设计.md](./混合检索与重排详细设计.md)》整理，是面向 Seahorse Agent 的可落地混合检索方案。目标是在不破坏现有微内核架构的前提下，补齐以下能力：

- 动态元数据过滤：支持租户、权限等系统字段，也支持业务自定义 metadata 字段；但自定义字段必须经过 Schema 注册、类型校验和后端能力声明。
- 向量检索闭环：补齐 query embedding 生成、过滤条件下推、命中元数据返回。
- BM25 / 关键词检索：通过关键词检索端口与适配器接入 OpenSearch / Elasticsearch / Lucene / PostgreSQL FTS。
...
- 不把 Milvus、PGVector、OpenSearch 等 SDK 泄漏到 `seahorse-agent-kernel`。
...
| 组件 | 当前状态 | 主要缺口 |
| --- | --- | --- |
| `SearchContext` | 有 `originalQuestion`、`rewrittenQuestion`、`intents`、`topK`、`metadata` | 缺少检索过滤、策略参数 |
| `RetrievedChunk` | 只有 `id`、`text`、`score` | 缺少 `kbId`、`docId`、`metadata`、通道分、融合分、重排分 |
| `VectorSearchRequest` | 已有 `filters: Map<String,Object>` | 适配器未消费，且缺少 Schema 约束 |
| `KernelMultiChannelRetrievalEngine` | 并行执行通道，合并后顺序执行后处理器 | 后处理器没有明确顺序 |
| `SearchChannelFeature` | 已支持通道插件 | 缺关键词/BM25 通道 |
| `SearchResultPostProcessorFeature` | 已支持后处理插件 | 缺元数据兜底过滤、RRF、Rerank、最终截断 |
| `RerankModelPort` | 已存在，OpenAI 兼容适配器已实现 `/rerank` | 未接入检索后处理链 |
| `EmbeddingModelPort` | 已存在 | 当前向量通道传空 vector，Milvus/PGVector 会返回空 |
| 向量适配器 | Milvus、PGVector、NoOp 已有 | 未下推过滤，未完整返回 metadata |
| `SearchChannelType` | 已有 `KEYWORD_ES`、`HYBRID` | 可复用，但建议后续补充 `KEYWORD_BM25` |
...
flowchart TD
    ...
    KP --> OS["OpenSearch / Elasticsearch / Lucene / PostgreSQL FTS"]
...
- 高频过滤字段必须声明索引策略。
- 未注册字段不能进入 Milvus / PGVector / OpenSearch 查询条件。
...
- `fieldKey` 是业务字段逻辑名，不是 Milvus JSON path、PG SQL 字段或 OpenSearch 字段名。
...
### 7.3 OpenSearch / Elasticsearch Mapping
...
- 本地开发可同步写入 Lucene / PostgreSQL FTS。
- 生产使用 Outbox 异步写 OpenSearch，避免搜索引擎抖动影响入库。
...
| `retrieval.keyword` | adapter、index、topK、hitCount、latency |
...
3. 新增 OpenSearch 适配器，Lucene 或 PostgreSQL FTS 作为可选 fallback。
4. 关键词索引通过 Outbox 异步同步。
5. 支持 Schema 驱动动态字段 mapping。

验收：

- 精确关键词问题可以通过 BM25 命中。
- OpenSearch 不可用时，不影响向量检索。
- 索引重建任务可按知识库/文档执行。
...
| 风险 | 影响 | 处理 |
| --- | --- | --- |
| 动态字段无限增长 | OpenSearch mapping 膨胀、查询慢 | Schema 注册、字段数量上限、索引策略审批 |
...
2. 再接 BM25 / 关键词通道，生产优先 OpenSearch，轻量环境可用 Lucene 或 PostgreSQL FTS。
