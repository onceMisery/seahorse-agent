...
- BM25 / 关键词检索：默认优先接入 Elasticsearch；PostgreSQL FTS 作为轻量 fallback；Lucene Embedded 已作为显式启用的低优先级嵌入式适配器落地；OpenSearch 保留为后续按企业环境引入的可插拔扩展。
...
| `SearchChannelFeature` | 已支持向量、意图定向和关键词/BM25 通道，关键词后端已覆盖 Elasticsearch、PostgreSQL FTS 和 Lucene Embedded | OpenSearch 可按同一端口后续接入 |
...
flowchart TD
    ...
    KP --> OS["Elasticsearch / PostgreSQL FTS / Lucene Embedded / OpenSearch(规划)"]
...
- 高频过滤字段必须声明索引策略。
- 未注册字段不能进入 Milvus / PGVector / Elasticsearch / Lucene Embedded / OpenSearch 查询条件。
...
- `fieldKey` 是业务字段逻辑名，不是 Milvus JSON path、PG SQL 字段、Elasticsearch 字段、Lucene 字段或 OpenSearch 字段名。
...
| 状态 | 适配器 | 模块建议 | 适用场景 |
| --- | --- | --- | --- |
| 已落地 / 默认生产推荐 | Elasticsearch | `seahorse-agent-adapter-search-elasticsearch` | 企业生产默认方案，提供成熟 BM25、中文分词、字段权重、过滤和高亮 |
| 已落地 / 轻量 fallback | PostgreSQL FTS | `seahorse-agent-adapter-search-postgres` | 本地开发、小规模部署、减少中间件依赖；排序使用 `ts_rank_cd`，不是严格 BM25 |
| 已落地 / 显式启用 | Lucene Embedded | `seahorse-agent-adapter-search-lucene` | 单机轻量方案，适合测试或私有化极简部署 |
| 规划 | OpenSearch | `seahorse-agent-adapter-search-opensearch` | 企业已有 OpenSearch 基础设施时按需接入 |
...
- 没有 Elasticsearch 集群的环境可以先启用 PostgreSQL FTS；单机轻量或私有化极简部署可显式启用 Lucene Embedded，后续仍可通过配置切换到 Elasticsearch 或 OpenSearch。
...
- 单机轻量或私有化极简部署可显式选择 Lucene Embedded；企业已有 OpenSearch 时，可通过后续新增 `KeywordIndexPort` / `KeywordSearchPort` 适配器切换。
...
默认推荐启用 `elasticsearch` 或 `postgres`；`lucene` 已作为低优先级嵌入式可插拔实现落地，适合测试、单机或私有化极简部署；`opensearch` 继续作为企业已有基础设施时的后续可选实现。
...
seahorse-agent:
  ...
  adapters:
    keyword-search:
      type: lucene
      lucene:
        index-directory: /var/lib/seahorse/lucene-keyword
        search-fields: content^3
    keyword-index:
      type: lucene
      lucene:
        index-directory: /var/lib/seahorse/lucene-keyword
...
- Lucene Embedded 已使用同一套 `KeywordSearchPort` / `KeywordIndexPort` 契约落地为显式启用的嵌入式实现；OpenSearch 仍按同一契约保留为后续规划，默认不作为主推荐链路。
...
5. Lucene Embedded 已作为低优先级可插拔适配器落地并需显式启用；OpenSearch 继续保留为后续可选扩展。
...
- 已提供 `KeywordSearchPort`、`KeywordIndexPort`、`KeywordSearchChannelFeature`，Spring Boot Starter 会在 Elasticsearch 或 Lucene 适配器被显式选择时注册对应实现，否则使用 JDBC PostgreSQL FTS fallback。
...
- `LuceneKeywordSearchAdapter` / `LuceneKeywordIndexAdapter` 已落地为嵌入式低优先级实现，通过 `seahorse-agent.adapters.keyword-search.type=lucene` 和 `seahorse-agent.adapters.keyword-index.type=lucene` 显式启用；索引目录由部署侧配置并持久化，动态 metadata 仍只消费 Filter Compiler 生成的 AST。
- 多通道检索编排器已记录 `retrieval.channel.completed` 观测事件，包含 channelName、channelType、hitCount、latencyMs、success 和异常类型；关键词/ES 通道失败时仍按空结果降级，同时保留失败证据。
- 当 Metadata Filter 因未知字段、不可过滤字段、操作符不允许或条件数超限而被拒绝时，检索编排器会先记录 `retrieval.metadata.filter.rejected` 观测事件，包含 schemaVersion、fieldKeys、reason 和异常类型，然后保持原有拒绝语义继续抛错。
- `MetadataGuardPostProcessorFeature` 已通过编排器记录 `retrieval.metadata.guard` 观测事件，包含 inputCount、outputCount、filteredCount、tenant、kb、耗时和成功状态，便于统计兜底过滤剔除量。
- 多通道检索编排器已记录 `retrieval.empty` 观测事件，区分 `no_enabled_channels`、`channels_returned_empty` 和 `post_processor_filtered_all`，为 `rag_retrieval_empty_total` 提供 stage/reason 维度来源。
- 元数据标准化字典管理已落地到 `MetadataDictionaryInboundPort` 和 `t_metadata_dictionary_item`，入库标准化阶段只消费启用字典项生成 canonical metadata；检索阶段仍只接收经 `MetadataFilterCompiler` 编译的过滤 AST，不允许字典管理接口直接承载动态过滤条件。
...
- 已新增知识库级检索评测集管理闭环，支持保存强类型 `RetrievalEvaluationCase` 列表、列表/详情/软删除和按已保存评测集运行现有评测服务。
- 已新增评测运行历史持久化，按评测集运行后保存汇总指标列与完整 `RetrievalEvaluationReport`，并提供运行列表/详情管理 API。
- 已支持基于已保存评测集直接做多策略 A/B 对比，避免管理端重复上传完整 cases，并将对比中的单策略报告继续沉淀到运行历史。
- 已新增对比批次历史持久化，保存单次 compare 的 winner/delta 快照，并提供 comparison 列表/详情查询接口。
...
| 风险 | 影响 | 处理 |
| --- | --- | --- |
| 动态字段无限增长 | Elasticsearch/OpenSearch mapping 膨胀或 Lucene 字段扩张，查询变慢 | Schema 注册、字段数量上限、索引策略审批 |
...
2. 再接 BM25 / 关键词通道，生产优先 Elasticsearch，轻量环境优先 PostgreSQL FTS；Lucene Embedded 已作为显式启用的嵌入式可插拔适配器落地，OpenSearch 保留为后续按需引入项。
