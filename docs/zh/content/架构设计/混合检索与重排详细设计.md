...
- BM25 / 关键词检索：新增关键词检索端口和检索通道，支持 OpenSearch/Elasticsearch/Lucene BM25，也支持 PostgreSQL 全文检索作为轻量 fallback。
...
- 不能把未实现能力写成现状；本文中的新增类、表和配置均为规划实现方案。
...
flowchart TD
    ...
    I --> VDB
    K --> KS["OpenSearch / Elasticsearch / Lucene / PostgreSQL FTS"]
...
| 适配器 | 模块建议 | 排名算法 | 适用场景 |
| --- | --- | --- | --- |
| OpenSearch | `seahorse-agent-adapter-search-opensearch` | Lucene BM25 | 企业生产、分布式、权限过滤复杂 |
| Elasticsearch | `seahorse-agent-adapter-search-elasticsearch` | Lucene BM25 | 已有 ES 基础设施 |
| Lucene Embedded | `seahorse-agent-adapter-search-lucene` | Lucene BM25 | 单机、私有化轻量部署 |
| PostgreSQL FTS | `seahorse-agent-adapter-search-postgres` | `ts_rank_cd`，不是严格 BM25 | 本地开发、小规模 fallback |

推荐生产优先 OpenSearch/Elasticsearch，因为 Lucene BM25 成熟、性能稳定，且支持中文分词插件、字段权重、过滤和高亮。

### 7.4 关键词索引文档模型

OpenSearch/Elasticsearch 索引建议：
...
优点：入库完成即可关键词检索。缺点：OpenSearch 慢或失败会影响入库链路。
...
如果生产使用 OpenSearch/Elasticsearch 做 BM25，`search_text` 可以只作为本地 fallback，不是必需字段。
...
3. 新增 OpenSearch/Elasticsearch 或 Lucene 适配器；PostgreSQL FTS 作为 fallback 可选。
4. 入库阶段同步或异步写关键词索引。
...
2. **再做 P2**：新增 `KeywordSearchPort` 和关键词通道，生产优先 OpenSearch/Elasticsearch BM25，轻量环境可用 PostgreSQL FTS fallback。
