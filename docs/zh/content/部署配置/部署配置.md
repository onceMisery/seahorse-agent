...
- [application.properties（启动器）](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties)
- [全量 Docker Compose 编排](file://docker-compose.full.yml)
- [全量 Docker Compose 编排](file://docker-compose.full.yml)
- [PostgreSQL 架构定义（PG）](file://resources/database/seahorse_init.sql)
- [PostgreSQL 初始化数据（PG）](file://resources/database/seahorse_init.sql)
...
- [application.properties（启动器）:1-2](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties#L1-L2)
- [MCP 服务器应用配置:1-7](file://seahorse-agent-mcp-server/src/main/resources/application.yml#L1-L7)
- [PostgreSQL 架构定义（PG）:1-800](file://resources/database/seahorse_init.sql#L1-L800)
...
### 开发环境搭建与本地运行
- JDK 与构建
  - 使用 Java 21，Spring Boot 3.5.7 版本，建议使用 Maven Wrapper 进行构建。
- 本地数据库初始化
...
- [application.properties（启动器）:1-2](file://seahorse-agent-spring-boot-autoconfigure/src/main/resources/application.properties#L1-L2)
- [PostgreSQL 架构定义（PG）:1-800](file://resources/database/seahorse_init.sql#L1-L800)
- [PostgreSQL 初始化数据（PG）:1-5](file://resources/database/seahorse_init.sql#L1-L5)
- [全量 Docker Compose 编排](file://docker-compose.full.yml)
- [全量 Docker Compose 编排](file://docker-compose.full.yml)
