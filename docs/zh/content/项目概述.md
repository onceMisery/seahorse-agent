...
> 当前状态提示：本文来自早期 RepoWiki 生成文档，已将旧 compose 文件引用对齐到当前根目录 `docker-compose.yml` / `docker-compose.full.yml`。若涉及默认端口、账号、Embedding 维度、接口路径或运行闭环，请优先以 `docs/README.md`、`docs/architecture/current-code-architecture.md` 和 `docs/USER_GUIDE.md` 为准。
...
- 严格遵循”加载记忆 → 激活四层记忆 → 查询优化 → 查询重写 → 解析意图 → 引导提示 → 系统仅回答 → 检索 → 空检索处理 → 流式 RAG 响应”的执行顺序；
...
**图表来源**
- [schema_table.sql:141-159](file://resources/database/seahorse_init.sql#L141-L159)
...
## 依赖关系分析
- 技术栈选择：
  - 后端：Java 21+、Spring Boot 3.5.7、MyBatis Plus、OkHttp、Sa-Token、Redisson、Pulsar、Milvus SDK、Apache Tika 等；
...
```mermaid
graph LR
JAVA["Java 21+"] --> SB["Spring Boot 3.5.7"]
SB --> MP["MyBatis Plus"]
SB --> OK["OkHttp"]
SB --> ST["Sa-Token"]
SB --> RS["Redisson"]
