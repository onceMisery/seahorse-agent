...
function readonlyPreset(card: RoleCardVO) {
  return truthy(card.readonly) || card.assetSource === "SYSTEM";
}
...
export function RoleCardPage() {
  ...
  const handleDelete = async (id: number | string) => {
    ...
    if (card && readonlyPreset(card)) {
      toast.error("系统预设角色卡不可删除");
      return;
    }
                  ...
                  <TableRow key={card.id}>
                    <TableCell>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {readonlyPreset(card) ? <Badge variant="outline">系统预设</Badge> : null}
                      </div>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          disabled={operatingId === card.id || readonlyPreset(card)}
                          title={readonlyPreset(card) ? "系统预设不可编辑" : undefined}
                        ...
                        <Button
                          ...
                          disabled={operatingId === card.id || readonlyPreset(card)}
                          title={readonlyPreset(card) ? "系统预设不可删除" : undefined}
