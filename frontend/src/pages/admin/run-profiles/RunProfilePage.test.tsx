...
describe("RunProfilePage", () => {
  ...
  it("shows system preset run profiles as readonly while keeping runtime actions available", async () => {
    serviceMocks.listRunProfiles.mockResolvedValueOnce([
      {
        id: 91,
        name: "深度研究",
    ...
    const row = (await screen.findByText("深度研究")).closest("tr");
    expect(row).not.toBeNull();
    const scope = within(row as HTMLElement);
    expect(scope.getByText("系统预设")).toBeInTheDocument();
    expect(scope.getByRole("button", { name: "设为默认" })).not.toBeDisabled();
    expect(scope.getByRole("button", { name: "预览" })).not.toBeDisabled();
    expect(scope.getByRole("button", { name: "发布门禁" })).not.toBeDisabled();
