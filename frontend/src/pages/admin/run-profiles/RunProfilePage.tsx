...
function readonlyPreset(profile: RunProfileVO) {
  return truthy(profile.readonly) || profile.assetSource === "SYSTEM";
}
...
export function RunProfilePage() {
  ...
  const handleEdit = async (profile: RunProfileVO) => {
    if (readonlyPreset(profile)) {
      toast.error("系统预设运行方案不可编辑");
      return;
    }
    try {
  ...
  const handleDelete = async (id: number | string) => {
    ...
    if (profile && readonlyPreset(profile)) {
      toast.error("系统预设运行方案不可删除");
      return;
    }
                  ...
                  <TableRow key={profile.id}>
                    <TableCell>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {readonlyPreset(profile) ? <Badge variant="outline">系统预设</Badge> : null}
                      </div>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          disabled={operatingId === profile.id || readonlyPreset(profile)}
                          title={readonlyPreset(profile) ? "系统预设不可编辑" : undefined}
                        ...
                        <Button
                          ...
                          disabled={operatingId === profile.id || readonlyPreset(profile)}
                          title={readonlyPreset(profile) ? "系统预设不可删除" : undefined}
