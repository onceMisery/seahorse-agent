-- Require a stable final Markdown outline for the GitHub visual project intro agent.
...
10. 输出前必须自检：硬性工具调用是否满足清单；所有关键结论是否能在 README、docs、源码或 web_fetch 材料中找到；是否包含图片引用和关键文件证据表。
    ...
    change_summary = 'Require stable final outline for GitHub visual project intro agent'
