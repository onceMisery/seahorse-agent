...
-- 元数据治理和关键词索引补偿按知识库 + 文档收敛候选分块，部分索引避免已软删数据参与维护路径。
...
COMMENT ON COLUMN t_metadata_extraction_result.status IS '结果状态：ACCEPT/ACCEPTED/REVIEW_REQUIRED/RE_EXTRACTING/QUARANTINE/QUARANTINED/REJECTED';
...
COMMENT ON COLUMN t_metadata_review_item.review_status IS '复核状态：PENDING/APPROVED/CORRECTED/RE_EXTRACTING/REJECTED/QUARANTINED';
