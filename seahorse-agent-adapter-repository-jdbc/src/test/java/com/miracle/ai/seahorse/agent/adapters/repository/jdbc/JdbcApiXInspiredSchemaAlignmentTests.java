...
class JdbcApiXInspiredSchemaAlignmentTests {
    ...
    void shouldKeepPackagedPostgresSqlAlignedWithApixInspiredTables() throws Exception {
        ...
        assertThat(sql).contains("preset_key");
        assertThat(sql).contains("role.requirement-analyst");
        assertThat(sql).contains("plan.deep-research");
        assertThat(sql).contains("asset_source");
        assertThat(sql).contains("readonly");
