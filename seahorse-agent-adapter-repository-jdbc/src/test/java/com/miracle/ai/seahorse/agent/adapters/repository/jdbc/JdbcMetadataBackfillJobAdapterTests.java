...
class JdbcMetadataBackfillJobAdapterTests {
    ...
    void shouldFilterBackfillJobsByGovernanceDetails() {
        ...
        MetadataBackfillJobPage page = adapter.page(new MetadataBackfillJobQuery(
                "tenant-1", "1", null, "pipe-1", "auditor", "doc-1",
    ...
    void shouldBuildBackfillOperationsOverview() {
        ...
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?), (?, ?, ?, ?), (?, ?, ?, ?)
                """,
                "review-1", "tenant-1", "1", "PENDING",
                "review-2", "tenant-1", "1", "PENDING",
                "review-3", "tenant-1", "1", "RE_EXTRACTING");
        jdbcTemplate.update("""
                INSERT INTO t_metadata_quarantine_item(id, tenant_id, kb_id, reason_code, resolved)
                VALUES (?, ?, ?, ?, ?), (?, ?, ?, ?, ?), (?, ?, ?, ?, ?)
                """,
                "q-1", "tenant-1", "1", "SCHEMA_MISSING", 0,
                "q-2", "tenant-1", "1", "LOW_CONFIDENCE", 0,
                "q-3", "tenant-1", "1", "SCHEMA_MISSING", 1);

        MetadataBackfillOperationsOverview overview = adapter.overview("tenant-1", "1");
