    ...
    void shouldPageToolInvocationAuditEntriesByFilters() {
        ...
        assertThat(rolloutPage.records()).singleElement().satisfies(entry -> {
            ...
            assertThat(entry.idempotencyKey())
                    .isEqualTo(ToolInvocationIdentity.digest("tenant-1", "run-1:invocation-4"));
        });
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sa_tool_invocation WHERE idempotency_key = 'run-1:invocation-4'",
                Long.class)).isZero();
    }

    @Test
    void shouldPersistentlyClaimToolInvocationIdempotencyPerTenant() {
        DriverManagerDataSource dataSource = dataSource("tool-invocation-idempotency");
        JdbcTemplate jdbcTemplate = new JdbcTemplate(dataSource);
        createToolInvocationSchema(jdbcTemplate);
        JdbcToolInvocationIdempotencyAdapter adapter = new JdbcToolInvocationIdempotencyAdapter(dataSource);
        Instant now = Instant.parse("2026-05-23T00:00:00Z");

        assertThat(adapter.tryClaim("tenant-1", "run-1:call-1", now)).isTrue();
        assertThat(adapter.tryClaim("tenant-1", "run-1:call-1", now.plusSeconds(1))).isFalse();
        assertThat(adapter.tryClaim("tenant-2", "run-1:call-1", now.plusSeconds(1))).isTrue();

        adapter.markCompleted("tenant-1", "run-1:call-1", now.plusSeconds(2));

        assertThat(jdbcTemplate.queryForObject(
                "SELECT status FROM sa_idempotency_key WHERE tenant_id = 'tenant-1'",
                String.class)).isEqualTo("COMPLETED");
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sa_idempotency_key",
                Long.class)).isEqualTo(2L);
        assertThat(jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM sa_idempotency_key WHERE idempotency_key = 'run-1:call-1'",
                Long.class)).isZero();
    }
