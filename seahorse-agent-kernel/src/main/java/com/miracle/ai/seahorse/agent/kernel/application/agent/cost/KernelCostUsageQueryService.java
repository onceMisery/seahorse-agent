...
public class KernelCostUsageQueryService implements CostUsageInboundPort {

    private final CostUsageRepositoryPort repository;

    public KernelCostUsageQueryService(CostUsageRepositoryPort repository) {
        this.repository = Objects.requireNonNull(repository, "repository must not be null");
    }

    @Override
    public CostUsageRecord append(CostUsageRecord record) {
        return repository.append(Objects.requireNonNull(record, "record must not be null"));
    }

    @Override
    public CostUsageAggregate aggregate(CostUsageQuery query) {
        return repository.aggregate(Objects.requireNonNull(query, "query must not be null"));
    }
}
