    ...
    public ToolInvocationResult invoke(ToolInvocationRequest request) {
        ...
        if (!isSupportedConversion(sourceFormat, targetFormat)) {
            return ToolInvocationResult.failed(
                    "sandbox_file_convert failed: supported conversions are csv/tsv to json, csv to xlsx, json to csv/tsv, txt to html, html to txt/docx, markdown/md to html/txt, docx/odt/odp/pdf to html/txt, docx/pptx/xlsx to pdf, pdf/pptx to png, pdf to ocr_txt, xlsx/ods to csv/html, and pptx to html/txt");
        }
