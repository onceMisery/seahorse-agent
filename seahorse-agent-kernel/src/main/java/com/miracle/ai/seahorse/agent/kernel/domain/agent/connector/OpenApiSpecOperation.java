...
public record OpenApiSpecOperation(String operationId,
    ...
    public OpenApiSpecOperation {
        operationId = Connector.trimToNull(operationId);
        method = Objects.requireNonNull(method, "method must not be null");
        path = Connector.requireText(path, "path must not be blank");
        summary = Connector.trimToNull(summary);
        description = Connector.trimToNull(description);
        schemaJson = Connector.trimToNull(schemaJson);
