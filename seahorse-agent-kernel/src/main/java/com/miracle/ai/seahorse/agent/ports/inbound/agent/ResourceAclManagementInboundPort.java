...
public interface ResourceAclManagementInboundPort {

    ResourceAclRule create(ResourceAclCreateCommand command);

    ResourceAclRule disable(String ruleId);

    ResourceAclRulePage page(ResourceAclQuery query);

    ResourceAclImportDryRunReport dryRunImport(ResourceAclImportDryRunCommand command);
