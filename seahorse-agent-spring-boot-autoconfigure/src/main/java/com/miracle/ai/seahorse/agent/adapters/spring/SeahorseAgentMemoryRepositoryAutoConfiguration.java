...
import com.miracle.ai.seahorse.agent.adapters.repository.jdbc.JdbcMemoryTraceRecorderAdapter;
...
public class SeahorseAgentMemoryRepositoryAutoConfiguration {
    ...
    public JdbcMemoryAggregationBufferAdapter seahorseJdbcMemoryAggregationBufferAdapter(
            ...
            @Value("${seahorse-agent.memory.aggregation.enabled:${seahorse.agent.memory.aggregation.enabled:false}}")
            boolean enabled,
            @Value("${seahorse-agent.memory.aggregation.idle-flush-millis:${seahorse.agent.memory.aggregation.idle-flush-millis:40000}}")
            long idleFlushMillis,
            @Value("${seahorse-agent.memory.aggregation.max-turns:${seahorse.agent.memory.aggregation.max-turns:10}}")
            int maxTurns,
            @Value("${seahorse-agent.memory.aggregation.max-tokens:${seahorse.agent.memory.aggregation.max-tokens:2000}}")
            int maxTokens,
            @Value("${seahorse-agent.memory.aggregation.max-context-blocks:${seahorse.agent.memory.aggregation.max-context-blocks:32}}")
            int maxContextBlocks,
            @Value("${seahorse-agent.memory.aggregation.buffer-ttl-millis:${seahorse.agent.memory.aggregation.buffer-ttl-millis:86400000}}")
            long bufferTtlMillis,
            @Value("${seahorse-agent.memory.aggregation.capture-on-error:${seahorse.agent.memory.aggregation.capture-on-error:false}}")
