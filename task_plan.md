...
- M3 已完成：新增 `KeywordSearchPort`、`KeywordIndexPort`、`KeywordSearchRequest` 和 `KeywordSearchChannelFeature`，starter 在存在 `KeywordSearchPort` 时注册关键词通道；已提供 JDBC/PostgreSQL FTS 轻量关键词 fallback、关键词索引 Outbox 异步化、Elasticsearch 生产适配器、kernel 级重建编排入口、Web 管理触发入口和索引失败观测。
- M3 尚未完成：计划型 Job 触发入口。
...
- [pending] M3 生产级关键词索引：计划型 Job 触发入口仍待后续落地。
